...
class InferEffects:
    ...
    def map_s(self, stmt):
        if isinstance(stmt, (LoopIR.Assign, LoopIR.Reduce)):
            ...
            return LoopIR.WriteConfig(
                stmt.config, stmt.field, stmt.rhs, eff, stmt.srcinfo
            )
            ...
            return LoopIR.Alloc(
                stmt.name, stmt.type, stmt.mem, eff_null(stmt.srcinfo), stmt.srcinfo
            )
        elif isinstance(stmt, LoopIR.WindowStmt):
...
class CheckEffects:
    ...
    def not_conflicts(self, iter, hi, e1, e2):
        ...
        iter_pred = SMT.And(
            SMT.And(SMT.LE(SMT.Int(0), iter1_smt), SMT.LT(iter1_smt, iter2_smt)),
            SMT.LT(iter2_smt, self.expr_to_smt(hi)),
        )
        self.solver.add_assertion(iter_pred)

        sub1 = {nm: E.Var(nm.copy(), T.index, null_srcinfo()) for nm in e1.names}
        sub1[iter] = E.Var(iter1, T.index, null_srcinfo())
        sub2 = {nm: E.Var(nm.copy(), T.index, null_srcinfo()) for nm in e2.names}
        ...
        loc1 = [self.expr_to_smt(i.subst(sub1)) for i in e1.loc]
        loc2 = [self.expr_to_smt(i.subst(sub2)) for i in e2.loc]
        ...
        if not self.solver.is_valid(loc_neq):
            ...
            self.err(
                e1,
                f"data race conflict with statement on "
    ...
    def not_conflicts_config(self, e1, e2):
        if e1.config == e2.config and e1.field == e2.field:
            ...
            if not self.solver.is_valid(disjoint):
                ...
                self.err(
                    e1,
                    f"conflict on configuration variable "
    ...
    def check_config_no_loop_depend(self, iter, eff, body):
        ...
        for ce in eff.config_writes:
            if iter in LoopIR_Dependencies((ce.config, ce.field), body).result():
                self.err(
                    ce,
                    f"The value written to config variable "
                    f"{ce.config.name()}.{ce.field} depends on "
                    f"the loop iteration variable {iter}",
                )
    ...
    def check_pos_size(self, expr):
        ...
        if not self.solver.is_valid(e_pos):
            ...
            self.err(
                expr,
                "expected expression to always be positive. "
    ...
    def check_non_negative(self, expr):
        ...
        if not self.solver.is_valid(e_nn):
            ...
            self.err(
                expr,
                "expected expression to always be non-negative. "
            ...
            if isinstance(stmt, LoopIR.If):
                ...
                for sig, arg in zip(stmt.f.args, stmt.args):
                    if sig.type.is_numeric():
                        ...
                        sig.type.is_indexable()
                        or sig.type == T.bool
                        or sig.type.is_stridable()
                    ):
                        # in this case we have a LoopIR expression...
                        subst[sig.name] = arg
                        if sig.type == T.size:
