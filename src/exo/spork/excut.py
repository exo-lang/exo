...
def str_id(s: str) -> int:
    ...
    _id = _str_id_table.get(s)
    if _id is None:
        assert valid_str(s), s
        _id = len(_str_id_table) + 1  # Reserve 0 for file name for now
        _str_id_table[s] = _id
    assert _id < (1 << 24)
    return _id
...
def generate_c_str_table_lines(src_filename):
    ...
    for s, i in _str_id_table.items():
        assert i < id_count
        strings[i] = s
    lines = ["// excut debug utility: generated by excut.generate_c_str_table_lines"]
    lines.append(
        "inline const uint32_t exo_excut_str_id_count = EXO_EXCUT_bENABLE_LOG ? %s : 0;"
        % id_count
    )
    ...
    def as_c_lines(self, *, py_format: bool) -> List[str]:
        ...
        for ptx_lineno, parsed_line in enumerate(self.parsed_lines):
            ...
            if logging:
                ...
                action_id = str_id(parsed_line.log_action)
                c_log_lines.append(
                    f"exo_excutLog.log_action({action_id}, 0, __LINE__);  // {parsed_line.log_action}"
                )
