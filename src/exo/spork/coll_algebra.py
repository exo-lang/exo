...
class CollDim:
    ...
    def get_offset_box_bit_union(self) -> int:
        bits = 0
        p = self.dim_thread_pitch
...
class CollTiling:
    ...
    def get_pow2_thread_alignment(self) -> int:
        """The set of active threads generated will always be aligned to this.

        Result is the highest possible A that is a power of 2 (at least 1) such
        that the set of threads generated by the resulting thread mapping function
        will always be expressible as the union of [Ap, Ap + A - 1] for p in P,
        P some set of ints.

        """
        bits = prod(self.get_domain())
        for d in self._dims:
            bits |= d.get_offset_box_bit_union()
        assert bits > 0
        align = 1
        while (align & bits) == 0:
            align *= 2
