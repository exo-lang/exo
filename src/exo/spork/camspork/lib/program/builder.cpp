...
void BodyBuilder::begin_orelse(ProgramBuilder *p_program_builder) {
  ...
  CAMSPORK_REQUIRE(body_of.type_id() == If_ID,
      "Must be defining If-statement body to call begin_orelse");
...
ProgramBuilder::ProgramBuilder() {
  ...
  for (uint32_t &magic : header_template.magic_numbers) {
    magic =
        ProgramHeader::expected_magic_numbers[&magic - &header_template
                                                            .magic_numbers[0]];
  }
  nursery.add_blob(sizeof header_template, &header_template);

  // Top-level BodyBuilder is special; we never pop it, and it is for building
  // the top-level stmt body.
...
void ProgramBuilder::finish() {
  ...
  CAMSPORK_REQUIRE_CMP(
      body_stack.size(), ==, 1, "missing pop_body() before finish()");
  ...
  for (const std::string &name : variable_slot_names) {
    var_configs.push_back(
        nursery.add_node<VarConfigRef>(VarConfig{}, name.size(), name.data()));
  }
  auto var_config_table = nursery.add_node<VarConfigTableRef>(
      VarConfigTable{}, var_configs.size(), var_configs.data());

  // Fill header; don't access it until we are sure we never call add_node
  // again!
...
ExprRef ProgramBuilder::add_ReadValue(
...
TrailingBarrierExprRef ProgramBuilder::add_TrailingBarrierExpr(
    Varname name, uint32_t num_idx, const ArriveIdx *idx) {
  check_not_finished();
  return nursery.add_node<TrailingBarrierExprRef>(
      TrailingBarrierExpr{name}, num_idx, idx);
}

template <typename ReadNode, typename MutateNode, typename IdxType>
StmtRef ProgramBuilder::add_SyncEnvAccess_impl(Varname name, size_t num_idx,
    const IdxType *idx, qual_bits_t initial_qual_bit,
    qual_bits_t extended_qual_bits, qual_bits_t atomic_qual_bits,
    uint32_t thread_access_granularity, uint32_t access_flags,
    TrailingBarrierExprRef trailing_barrier_expr) {
  static_assert(access_flag_all_bits == 15, "update me");
  CAMSPORK_REQUIRE_CMP(
      access_flags, <=, access_flag_all_bits, "unknown access_flags bit set");
  ...
  auto impl = [&](auto node) {
    ...
    if constexpr (node.is_mutate) {
      ...
      CAMSPORK_REQUIRE_CMP(
          atomic_qual_bits, ==, 0, "atomics must have access_flag_mutate set");
    }
...
StmtRef ProgramBuilder::add_SyncEnvAccess(Varname name, size_t num_idx,
    const ExprRef *idx, qual_bits_t initial_qual_bit,
    qual_bits_t extended_qual_bits, qual_bits_t atomic_qual_bits,
    uint32_t thread_access_granularity, uint32_t access_flags,
    TrailingBarrierExprRef trailing_barrier_expr) {
  return add_SyncEnvAccess_impl<SyncEnvReadSingle, SyncEnvMutateSingle>(name,
      num_idx, idx, initial_qual_bit, extended_qual_bits, atomic_qual_bits,
      thread_access_granularity, access_flags, trailing_barrier_expr);
}

StmtRef ProgramBuilder::add_SyncEnvAccess(Varname name, size_t num_idx,
    const OffsetExtentExpr *idx, qual_bits_t initial_qual_bit,
    qual_bits_t extended_qual_bits, qual_bits_t atomic_qual_bits,
    uint32_t thread_access_granularity, uint32_t access_flags,
    TrailingBarrierExprRef trailing_barrier_expr) {
  return add_SyncEnvAccess_impl<SyncEnvReadWindow, SyncEnvMutateWindow>(name,
      num_idx, idx, initial_qual_bit, extended_qual_bits, atomic_qual_bits,
      thread_access_granularity, access_flags, trailing_barrier_expr);
}

StmtRef ProgramBuilder::add_SyncEnvAccess(Varname name, size_t num_idx,
    const ArriveIdx *idx, qual_bits_t initial_qual_bit,
    qual_bits_t extended_qual_bits, qual_bits_t atomic_qual_bits,
    uint32_t thread_access_granularity, uint32_t access_flags,
    TrailingBarrierExprRef trailing_barrier_expr) {
  return add_SyncEnvAccess_impl<SyncEnvReadMulticast, SyncEnvMutateMulticast>(
      name, num_idx, idx, initial_qual_bit, extended_qual_bits,
      atomic_qual_bits, thread_access_granularity, access_flags,
      trailing_barrier_expr);
}

StmtRef ProgramBuilder::add_SyncEnvFreeShard(Varname name, size_t num_idx,
    const ExprRef *idx, qual_bits_t extended_qual_bits) {
  return append_impl(SyncEnvFreeShard{name, extended_qual_bits}, num_idx, idx);
}

StmtRef ProgramBuilder::add_MutateValue(
    Varname name, size_t num_idx, const ExprRef *idx, binop op, ExprRef rhs) {
  return append_impl(MutateValue{name, op, rhs}, num_idx, idx);
}

StmtRef ProgramBuilder::add_Fence(qual_bits_t L1_qual_bits,
    qual_bits_t L2_full_qual_bits, qual_bits_t L2_temporal_qual_bits) {
  CAMSPORK_REQUIRE_CMP(L2_full_qual_bits, ==,
      L2_full_qual_bits & L2_temporal_qual_bits,
      "L2_full_qual_bits must be a subset of L2_temporal_qual_bits");
  return append_impl(
      Fence{L1_qual_bits, L2_full_qual_bits, L2_temporal_qual_bits});
}

StmtRef ProgramBuilder::add_Arrive(qual_bits_t L1_qual_bits, Varname name,
    uint32_t num_idx, const ArriveIdx *idx) {
  return append_impl(Arrive{name, L1_qual_bits}, num_idx, idx);
}

StmtRef ProgramBuilder::add_Await(Varname name, uint32_t num_idx,
    const ExprRef *idx, uint32_t L2_full_qual_bits,
    uint32_t L2_temporal_qual_bits, int32_t N) {
  CAMSPORK_REQUIRE_CMP(L2_full_qual_bits, ==,
      L2_full_qual_bits & L2_temporal_qual_bits,
      "L2_full_qual_bits must be a subset of L2_temporal_qual_bits");
  return append_impl(
      Await{name, L2_full_qual_bits, L2_temporal_qual_bits, N}, num_idx, idx);
}

StmtRef ProgramBuilder::add_ValueEnvAlloc(
    Varname name, size_t num_dims, const ExprRef *extent) {
  return append_impl(ValueEnvAlloc{name}, num_dims, extent);
}

StmtRef ProgramBuilder::add_SyncEnvAlloc(
    Varname name, size_t num_dims, const ExprRef *extent) {
  return append_impl(SyncEnvAlloc{name}, num_dims, extent);
}

StmtRef ProgramBuilder::add_ExpectSyncEnvAlloc(
    Varname name, size_t num_dims, const ExprRef *extent) {
  return append_impl(ExpectSyncEnvAlloc{name}, num_dims, extent);
}

StmtRef ProgramBuilder::add_BarrierEnvAlloc(
    Varname name, size_t num_dims, const ExprRef *extent) {
...
StmtRef ProgramBuilder::add_JoinThreads() { return append_impl(JoinThreads{}); }

StmtRef ProgramBuilder::push_If(ExprRef cond) {
  return push_impl(If{cond, {}, {}});
}

void ProgramBuilder::begin_orelse() { body_stack.back().begin_orelse(this); }
...
StmtRef ProgramBuilder::push_ThreadsFor(Varname iter, ExprRef lo, ExprRef hi,
    uint32_t dim_idx, uint32_t offset, uint32_t box) {
  ThreadsFor node;
  node.iter = iter;
  node.lo = lo;
  node.hi = hi;
...
void ProgramBuilder::check_not_finished() const {
  CAMSPORK_REQUIRE(
      !p_shared_finished_buffer, "Cannot modify program after finish()");
}
...
void ProgramBuilder::pop_body(StmtRef *out_body, StmtRef *out_orelse) {
  CAMSPORK_REQUIRE_CMP(
      body_stack.size(), >=, 2, "Cannot pop last statement body; use finish()");
...
StmtRef ProgramBuilder::body_to_nursery(const std::vector<StmtRef> &stmts) {
  // XXX this could cause the nursery to realloc, which invalidates all Node
  // pointers.
  check_not_finished();
  if (stmts.size() == 1) {
    return stmts[0];
...
int camspork_ProgramBuilder_is_finished(
...
camspork_RawVarname camspork_add_variable(
    camspork::ProgramBuilder *p_builder, const char *p_name) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawExprRef camspork_add_Const(
    camspork::ProgramBuilder *p_builder, camspork::value_t value) {
  CAMSPORK_API_PROLOGUE
  return p_builder->add_Const(value);
  CAMSPORK_API_EPILOGUE(camspork::ExprRef())
}

camspork_RawExprRef camspork_add_USub(
    camspork::ProgramBuilder *p_builder, camspork_RawExprRef arg) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawTrailingBarrierExprRef camspork_add_TrailingBarrierExpr(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
...
static camspork::TrailingBarrierExprRef get_trailing_barrier_expr(
  ...
  return trailing_barrier_expr ? *trailing_barrier_expr
                               : camspork::TrailingBarrierExprRef();
}

camspork_RawStmtRef camspork_add_SyncEnvAccessSingle(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_idx, const camspork::ExprRef *idx,
    camspork::qual_bits_t initial_qual_bit,
    camspork::qual_bits_t extended_qual_bits,
    camspork::qual_bits_t atomic_qual_bits, uint32_t thread_access_granularity,
    uint32_t access_flags,
    const camspork::TrailingBarrierExprRef *trailing_barrier_expr) {
  CAMSPORK_API_PROLOGUE
  return p_builder->add_SyncEnvAccess(name, num_idx, idx, initial_qual_bit,
      extended_qual_bits, atomic_qual_bits, thread_access_granularity,
      access_flags, get_trailing_barrier_expr(trailing_barrier_expr));
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_add_SyncEnvAccessWindow(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_idx, const camspork::OffsetExtentExpr *idx,
    camspork::qual_bits_t initial_qual_bit,
    camspork::qual_bits_t extended_qual_bits,
    camspork::qual_bits_t atomic_qual_bits, uint32_t thread_access_granularity,
    uint32_t access_flags,
...
camspork_RawStmtRef camspork_add_SyncEnvAccessMulticast(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_idx, const camspork::ArriveIdx *idx,
    camspork::qual_bits_t initial_qual_bit,
    camspork::qual_bits_t extended_qual_bits,
...
camspork_RawStmtRef camspork_add_SyncEnvFreeShard(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_idx, const camspork::ExprRef *idx,
    camspork::qual_bits_t extended_qual_bits) {
  CAMSPORK_API_PROLOGUE
  return p_builder->add_SyncEnvFreeShard(
      name, num_idx, idx, extended_qual_bits);
  CAMSPORK_API_EPILOGUE(camspork::StmtRef());
}

camspork_RawStmtRef camspork_add_MutateValue(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_idx, const camspork::ExprRef *idx, camspork::binop op,
    camspork::ExprRef rhs) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_Fence(camspork::ProgramBuilder *p_builder,
    camspork::qual_bits_t L1_qual_bits, camspork::qual_bits_t L2_full_qual_bits,
    camspork::qual_bits_t L2_temporal_qual_bits) {
  CAMSPORK_API_PROLOGUE
  return p_builder->add_Fence(
      L1_qual_bits, L2_full_qual_bits, L2_temporal_qual_bits);
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_add_Arrive(camspork::ProgramBuilder *p_builder,
    camspork::qual_bits_t L1_qual_bits, camspork_RawVarname name,
    uint32_t num_idx, const camspork::ArriveIdx *idx) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_Await(camspork::ProgramBuilder *p_builder,
  ...
  CAMSPORK_API_PROLOGUE
  return p_builder->add_Await(
      name, num_idx, idx, L2_full_qual_bits, L2_temporal_qual_bits, N);
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_add_ValueEnvAlloc(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_dims, const camspork::ExprRef *extent) {
  CAMSPORK_API_PROLOGUE
  return p_builder->add_ValueEnvAlloc(name, num_dims, extent);
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_add_SyncEnvAlloc(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_dims, const camspork::ExprRef *extent) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_ExpectSyncEnvAlloc(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_dims, const camspork::ExprRef *extent) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_BarrierEnvAlloc(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name,
    uint32_t num_dims, const camspork::ExprRef *extent) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_DataFree(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_BarrierFree(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname name) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_add_JoinThreads(
    camspork::ProgramBuilder *p_builder) {
  CAMSPORK_API_PROLOGUE
  return p_builder->add_JoinThreads();
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_push_If(
    camspork::ProgramBuilder *p_builder, camspork::ExprRef cond) {
  CAMSPORK_API_PROLOGUE
...
camspork_RawStmtRef camspork_push_ThreadsFor(
    camspork::ProgramBuilder *p_builder, camspork_RawVarname iter,
    camspork::ExprRef lo, camspork::ExprRef hi, uint32_t dim_idx,
    uint32_t offset, uint32_t box) {
  CAMSPORK_API_PROLOGUE
  return p_builder->push_ThreadsFor(iter, lo, hi, dim_idx, offset, box);
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_push_ParallelBlock(
    camspork::ProgramBuilder *p_builder, uint32_t dim, const uint32_t *domain) {
  CAMSPORK_API_PROLOGUE
  return p_builder->push_ParallelBlock(dim, domain);
  CAMSPORK_API_EPILOGUE(camspork::StmtRef())
}

camspork_RawStmtRef camspork_push_DomainReshape(
    camspork::ProgramBuilder *p_builder, uint32_t dim, const uint32_t *domain) {
  CAMSPORK_API_PROLOGUE
