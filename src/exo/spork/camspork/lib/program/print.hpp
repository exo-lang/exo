...
class ProgramPrinter {
  Stream &stream;
  StmtRefRemarksCallback
      &remarks_callback;  // StmtRef -> sequence_container<obj> where we can do
                          // Stream << obj.
  size_t buffer_size;
...
public:
  ...
  ProgramPrinter(Stream &_stream, StmtRefRemarksCallback &_remarks_callback,
    ...
    const ProgramHeader &header =
        ProgramHeader::validate(buffer_size, program_buffer);
  ...
  void operator()(const VarConfig *p_config) {
    ...
    std::string prefix(&node_vla_get_unsafe(p_config, 0),
        &node_vla_get_unsafe(p_config, strlen));
    for (int i = 0;; ++i) {
  ...
  void operator()(const Const *node) { *this << node->value; }
  ...
  void operator()(const BinOp *node) {
    // Just parenthesize everything for now, except maybe for top-level BinOp
    // (binop_no_parens_flag).
  ...
  void operator()(const Fence *node) {
    ...
    *this << ", " << node->L2_full_qual_bits << ", "
          << node->L2_temporal_qual_bits << ")\n";
  }
  ...
  void operator()(const ThreadsFor *node) {
    ...
    *this << "with b.ThreadsFor(" << node->iter << ", " << node->lo;
    binop_no_parens_flag = true;
    *this << ", " << node->hi << ", " << node->dim_idx << ", " << node->offset
          << ", " << node->box << "):\n";
    indent_levels++;
    *this << node->body;
    indent_levels--;
    ...
    for (uint32_t expr_idx = 0; expr_idx < 32; ++expr_idx) {
      if (dim > 0) {
        CAMSPORK_REQUIRE_CMP(
            dim, <=, 32, "sorry, too many dims in Arrive to handle");
...
void print_program(
    Stream &stream, size_t buffer_size, const char *program_buffer) {
  auto no_remarks = [](StmtRef) { return std::array<char, 0>{}; };
  ProgramPrinter<Stream, decltype(no_remarks)>(
      stream, no_remarks, buffer_size, program_buffer);
}

template <typename Stream, typename StmtRefRemarksCallback>
void print_program(Stream &stream, StmtRefRemarksCallback &remarks_callback,
    size_t buffer_size, const char *program_buffer) {
  ProgramPrinter<Stream, StmtRefRemarksCallback>(
      stream, remarks_callback, buffer_size, program_buffer);
}
...
// 0 = error, 1 = success.
CAMSPORK_EXPORT int camspork_thread_local_print_program(
    size_t buffer_size, const void *program_buffer);
CAMSPORK_EXPORT int camspork_thread_local_print_program_with_remarks(
