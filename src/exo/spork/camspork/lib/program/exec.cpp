...
public:
  ...
  void exec_sync_env_impl(
    ...
    // update.
    using Input = std::conditional_t<node->is_window, AssignmentRecordWindow,
        assignment_record_id *>;
    using InputList = std::conditional_t<node->is_multicast, std::vector<Input>,
        std::array<Input, 1>>;
    ...
    if constexpr (node->is_multicast) {
      static_assert(
          !node->is_window, "we can only multicast a single position");
