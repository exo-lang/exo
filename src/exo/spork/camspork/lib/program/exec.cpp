...
public:
  ...
  [[nodiscard]] SwapThreadCuboid(
      ThreadCuboid *p_cuboid, ThreadCuboid new_cuboid) {
    saved = *p_cuboid;
    p_restore = p_cuboid;
    *p_cuboid = new_cuboid;
  };
  ...
  ~SwapThreadCuboid() { *p_restore = saved; }
};
...
protected:
  ...
  // if AllowLog && excut_file, then the per-stmt/per-expr callbacks in
  // ProgramExec may push excut actions to log. These should be flushed for each
  // statement using flush_excut_log, if applicable.
...
// Borrowed reference wrapper around ProgramEnv, to implement actual
...
class ProgramExec : public ProgramExecLogBase<AllowLog> {
  ...
  std::vector<extent_t>
      single_position_all_ones;  // [1 for x in single_position_filter.idx]
...
public:
  ProgramExec(ProgramEnv *p_self, SinglePositionFilter _single_position_filter)
    ...
    if (single_position_filter) {
      single_position_all_ones =
          std::vector<extent_t>(single_position_filter.idx.size(), 1);
    }
  }

  ProgramExec(ProgramEnv *p_self, const char *p_excut_filename,
      SinglePositionFilter _single_position_filter)
      : ProgramExec(p_self, std::move(_single_position_filter)) {
    static_assert(
        AllowLog, "Can't open excut log file if C++ functionality not enabled");
    if (p_excut_filename) {
      FILE *&file = this->excut_file;
      file = fopen(p_excut_filename, "w");
      if (!file) {
        throw std::runtime_error(
            std::string(p_excut_filename) + ": " + strerror(errno));
      }
      fprintf(file, "[\n");
    }
  }
  ...
  struct ExprIterator {
    ...
    value_t operator*() const { return p_exec->eval(*p_node_ref); }
  ...
  void eval_tmp_extent(const Node *node) {
    ...
    for (uint32_t i = 0; i < dim; ++i) {
      if constexpr (std::is_same_v<typename Node::camspork_vla_type,
                        OffsetExtentExpr>) {
  ...
  void eval_tmp_offset(const Node *node) {
    ...
    for (uint32_t i = 0; i < dim; ++i) {
      ...
      } else if constexpr (std::is_same_v<typename Node::camspork_vla_type,
                               ArriveIdx>) {
  ...
  static void print_idx_helper(
      Stream &stream, const std::vector<extent_t> &idx) {
    if (!idx.empty()) {
      stream << '[' << idx[0];
  ...
  [[nodiscard]] bool filter_single_position_input(Varname name,
      const VarSlotEntry<assignment_record_id> &slot, Input *p_input) {
    if (!single_position_filter) {
      return true;
    }
    ...
    if constexpr (std::is_same_v<Input, AssignmentRecordWindow>) {
      ...
      CAMSPORK_REQUIRE_CMP(size_t(window.end_offset - window.begin_offset), ==,
          dim, "internal error");
      CAMSPORK_REQUIRE_CMP(
          size_t(window.end_inner_extent - window.begin_inner_extent), ==, dim,
          "internal error");
      for (size_t dim_i = 0; dim_i < dim; ++dim_i) {
        static_assert(
            std::is_unsigned_v<extent_t>, "Relies on wraparound to work");
        const extent_t tmp =
            single_position_filter.idx[dim_i] - window.begin_offset[dim_i];
      ...
      CAMSPORK_REQUIRE_CMP(
          single_position_all_ones.size(), ==, dim, "internal error");
      ...
      // Note, we do not assume in this code that the single_position_filter.idx
      // is in-bounds for the current size of the allocation.
  ...
  template <typename Node>
  auto prepare_logger(
      const Node *node, const ThreadCuboid &thread_cuboid, StmtRef stmt) {
    using Logger =
        std::conditional_t<AllowLog, SyncvLogRequest, decltype(nullptr)>;
    Logger logger{};
    if constexpr (AllowLog) {
      if constexpr (!std::is_same_v<Node, Fence> &&
                    !std::is_same_v<Node, JoinThreads>) {
        logger.var_str_name = env.str_name(node->name);
      }
      logger.p_excut_actions =
          this->excut_file ? &this->excut_actions : nullptr;
      if (env.history_enable) {
        static_assert(sizeof(VisRecordHistoryLog::stmt_id_bits_t) ==
                      sizeof(stmt.raw_data));
  ...
  void exec_sync_env_impl(
      const SyncEnvAccessNode<IsMutate, IsWindow, IsMulticast> *node,
      StmtRef stmt_ref) {
    ...
    CAMSPORK_REQUIRE_CMP(IsMutate, ==,
        bool(node->access_flags & access_flag_mutate),
        "implementation didn't set static IsMutate type to match requested "
        "mutate flag");
    ...
    // If multicasting, the input is a list of individual assignment records to
    // update.
    using Input = std::conditional_t<NodeType::is_window,
        AssignmentRecordWindow, assignment_record_id *>;
    using InputList = std::conditional_t<NodeType::is_multicast,
        std::vector<Input>, std::array<Input, 1>>;
    ...
    if constexpr (NodeType::is_multicast) {
      static_assert(
          not NodeType::is_window, "we can only multicast a single position");
      ...
      CAMSPORK_REQUIRE_CMP(
          tmp_extent.size(), ==, dim, "Wrong dimensionality for indexing");
      CAMSPORK_REQUIRE_CMP(
          tmp_offset.size(), ==, dim, "Wrong dimensionality for indexing");
      ...
      for (size_t i = 0; i < dim; ++i) {
        if (tmp_offset[i] >= alloc_extent[i]) {
          ...
          tmp_extent[i] =
              std::min(tmp_extent[i], alloc_extent[i] - tmp_offset[i]);
        }
      }
    ...
    for (Input &input : input_list) {
      ...
      if constexpr (AllowLog) {
        if constexpr (not NodeType::is_window) {
          logger.idx_for_single =
              slot.idx_from_linear(size_t(input - slot.data()));
        }
      }

      // Call into syncv table (a lot of duplicated code with SyncEnvFreeShard,
      // not great)
      try {
  ...
  void exec_impl(const SyncEnvFreeShard *node) {
    ...
    SyncvAccessInfo access{};
    access.is_ooo = false;
    access.is_convergent =
        false;  // All threads must be prepared to free the memory.
    ...
    CAMSPORK_REQUIRE_CMP(tmp_offset.size(), <=, alloc_dim,
        "Too many dimensions for SyncEnvFreeShard");
    ...
    // Call into syncv table (a lot of duplicated code with SyncEnvAccessNode,
    // not great).
    try {
      if (!filter_single_position_input(node->name, slot, &input)) {
        // Skip if instructed to by SinglePositionFilter.
        ...
        on_check_free(
            env.p_syncv_table.get(), input, thread_cuboid, access, logger);
      }
    } catch (const SyncvCheckFail &exc) {
  ...
  void exec_impl(const MutateValue *node) {
    value_t &lhs = env.value_slot(node->name)
                       .idx(expr_vla_begin(node), expr_vla_end(node));
  ...
  void exec_impl(const Fence *node) {
    ...
    const ThreadCuboid &thread_cuboid = env.prepare_thread_cuboid();
    on_fence(env.p_syncv_table.get(), thread_cuboid, param,
        prepare_logger(node, thread_cuboid));
    env.maybe_syncv_debug_validate();
  }
  ...
  void exec_impl(const Arrive *node) {
    ...
    // Pass to SyncvTable.
    const ThreadCuboid &thread_cuboid = env.prepare_thread_cuboid();
    on_arrive(env.p_syncv_table.get(), thread_cuboid, param,
        prepare_logger(node, thread_cuboid));
    env.maybe_syncv_debug_validate();
  }

  template <typename Node, typename Slot, typename Callback>
  void eval_tmp_offset_multicast(
      const Node *node, Slot &slot, Callback &&callback) {
    // Requires Node has ArriveIdx as VLA type.
    // Fill tmp_offset based on the index, and resolve multicasting.
    // callback(slot, linear_idx) is called for each (linearized, C order)
    // position covered by multicasting.
    const std::vector<extent_t> &extent = slot.extent();
    const uint32_t dim = node->camspork_vla_size;
    CAMSPORK_REQUIRE_CMP(dim, ==, extent.size(), "dimension mismatch");

    // Evaluate concrete indices (for barriers this is the "home barrier"
    // index).
    eval_tmp_offset(node);

    auto recurse = [&](uint32_t dim_idx, uint32_t partial_idx,
                       uint32_t equality_mask, auto recurse) {
      if (dim_idx >= dim) {
        if (equality_mask != 0) {
      ...
      for (extent_t i = 0; i < extent_coord; ++i) {
        const uint32_t tmp_mask =
            (i == var_value) ? ~uint32_t(0) : arrive_idx.multicast_per_expr;
        recurse(dim_idx + 1, partial_idx * extent_coord + i,
            equality_mask & tmp_mask, recurse);
      }
    };
  ...
  barrier_id fill_tmp_offset_barriers(const Node *node) {
    ...
    const barrier_id home_barrier =
        _slot.idx(tmp_offset.begin(), tmp_offset.end());
    return home_barrier;
  }
  ...
  void exec_impl(const Await *node) {
    ...
    // Pass to SyncvTable.
    const ThreadCuboid &thread_cuboid = env.prepare_thread_cuboid();
    on_await(env.p_syncv_table.get(), thread_cuboid, param,
        prepare_logger(node, thread_cuboid));
    env.maybe_syncv_debug_validate();
  }
  ...
  void exec_impl(const JoinThreads *node) {
    ...
    const ThreadCuboid &thread_cuboid = env.prepare_thread_cuboid();
    on_join_threads(env.p_syncv_table.get(), thread_cuboid, param,
        prepare_logger(node, thread_cuboid));
  }
  ...
  void exec_impl(const SyncEnvAlloc *node) {
    ...
    // This only runs if the resize(...) caused a reallocation, so we also rely
    // on clear_sync_env calling clear_visibility to 0 things. This is OK even
    // if size < capacity, since the ones in [size, capacity) would have been
    // 0'd in the past at some point, since it was either never used (0-init by
    // alloc) or free'd after last use (clear_sync_env).
  }
  ...
  void exec_impl(const ExpectSyncEnvAlloc *node) {
    if (single_position_filter.accepts_name(node->name)) {
      ...
      if (slot.extent() != tmp_extent) {
        CAMSPORK_REQUIRE(
            0, "ExpectSyncEnvAlloc saw wrong size for sync env allocation");
      }
    }
  }
  ...
  void log_barrier_helper(const std::string &var_str_name,
      const VarSlotEntry<barrier_id> &slot, std::vector<extent_t> idx) {
  ...
  struct BodyExecImpl {
    ...
    void operator()(const Stmt *node) {
      ...
      // Since this is per-Stmt type, the branch predictor may learn
      // correlations of what the next statement type is with respect to this
      // statement type.
  ...
  void exec_impl(const TasksFor *node) {
    ...
    for (value_t i = lo; i < hi; ++i) {
      ...
      // This avoids wasting task_index values on every level of the TasksFor
      // loop nest.
  ...
  void exec_impl(const ThreadsFor *node) {
    ...
    // This shouldn't hard to change, but just test it quickly if you change
    // this.
    CAMSPORK_REQUIRE_CMP(
        lo, ==, 0, "Expected ThreadsFor loop to start from 0 for now");

    // Restores thread cuboid before returning (we have to update task index
    // before saving, otherwise we might restore an incorrect task index).
    env.prepare_thread_cuboid();
    SwapThreadCuboid swap(&env.raw_thread_cuboid);

    CAMSPORK_REQUIRE_CMP(dim_idx, <, env.raw_thread_cuboid.dim(),
        "ThreadsFor::dim_idx out of range");
    CAMSPORK_REQUIRE_CMP(offset_c + (hi - lo) * box_c, <=,
        env.raw_thread_cuboid.box()[dim_idx],
        "ThreadsFor consumes more threads than exists in the current thread "
        "box");
  ...
  void exec_impl(const ParallelBlock *node) {
    ...
    // Execute body with new thread cuboid, and restore before returning
    // (~SwapThreadCuboid).
    env.prepare_thread_cuboid();
  ...
  void exec_impl(const DomainReshape *node) {
    ThreadCuboid new_cuboid =
        env.prepare_thread_cuboid();  // Must update task_index here!
  ...
  void flush_excut_log() {
    if constexpr (AllowLog) {
      if (this->excut_file) {
        std::vector<std::unique_ptr<ExcutBaseAction>> &actions =
            this->excut_actions;
        FILE *file = this->excut_file;
        for (const auto &p_action : actions) {
          ...
          fprintf(file, ", \"%s\", %u, %u, \"%s\", %i]\n",
              is_cpu ? "cpu" : "am_threads", cuboid.task_index, local_tid,
              source, line);
          first_time = false;
        }
        actions.clear();
  ...
  extent_t eval_extent_t(ExprRef e) const {
    ...
    CAMSPORK_REQUIRE_CMP(
        v, >=, 0, "Negative value used as array index or extent");
    return extent_t(v);
  }

  CAMSPORK_EXEC_ALWAYS_INLINE
  value_t operator()(const ReadValue *node) const {
    return env.value_slot(node->name)
        .idx(expr_vla_begin(node), expr_vla_end(node));
  }

  CAMSPORK_EXEC_ALWAYS_INLINE
  value_t operator()(const Const *node) const { return node->value; }

  value_t operator()(const USub *node) const { return -eval(node->arg); }

  value_t operator()(const BinOp *node) const {
    CAMSPORK_REQUIRE_CMP(int(node->op), !=, int(binop::Assign),
        "binop::Assign is only allowed in MutateValue");
  ...
  void operator()(const VarConfig *config) {
    ...
    env.var_slots.push_back(
        {std::string(p_str, p_str + num_bytes), {}, {}, {}});
  }
};
// End ProgramExec
...
ProgramEnv::ProgramEnv(size_t buffer_size, std::shared_ptr<const char[]> buffer)
    : program_buffer_size(buffer_size),
      ...
      raw_thread_cuboid(
          ThreadCuboid::full(&static_uint32_max, 1 + &static_uint32_max)) {
  ProgramExec<false>(this, no_single_position_filter)
      .init_vars(header.var_config_table);
};
...
void ProgramEnv::exec(StmtRef stmt, const char *p_excut_filename,
  ...
  if (p_excut_filename || history_enable) {
    ProgramExec<true>(this, p_excut_filename, std::move(single_position_filter))
        .exec(stmt);
  } else {
...
void ProgramEnv::set_history_enable(bool flag) { history_enable = flag; }
...
void ProgramEnv::stream_program_remarks(std::ostream &stream) {
  ...
  auto get_remarks =
      [&remarks_map, &empty](
          camspork::StmtRef stmt) -> const std::vector<const char *> & {
  ...
  print_program(
      stream, get_remarks, program_buffer_size, p_program_buffer.get());
}

}  // end namespace camspork

camspork::ProgramEnv *camspork_new_ProgramEnv(
...
camspork::ProgramEnv *camspork_copy_ProgramEnv(
...
int camspork_exec_top(camspork::ProgramEnv *p_env, const char *p_excut_filename,
  ...
  p_env->exec(p_excut_filename,
      camspork::SinglePositionFilter{single_position_name, {idx, idx + dims}});
  return 1;
...
int camspork_alloc_values(camspork::ProgramEnv *p_env, camspork::Varname name,
    uint32_t dims, const camspork::extent_t *p_extent) {
  CAMSPORK_API_PROLOGUE
  p_env->alloc_values(
      name, std::vector<camspork::extent_t>(p_extent, p_extent + dims));
  return 1;
  CAMSPORK_API_EPILOGUE(0)
}

int camspork_alloc_scalar_value(camspork::ProgramEnv *p_env,
...
int camspork_alloc_sync(camspork::ProgramEnv *p_env, camspork::Varname name,
    uint32_t dims, const camspork::extent_t *p_extent) {
  CAMSPORK_API_PROLOGUE
  p_env->alloc_sync(
      name, std::vector<camspork::extent_t>(p_extent, p_extent + dims));
...
int camspork_read_value(const camspork::ProgramEnv *p_env,
...
int camspork_set_value(camspork::ProgramEnv *p_env, camspork::Varname name,
    uint32_t dims, const camspork::value_t *idxs, camspork::value_t arg) {
  CAMSPORK_API_PROLOGUE
...
int camspork_set_debug_validation_enable(
...
CAMSPORK_EXPORT const char *camspork_get_remark(
    const camspork::ProgramEnv *p_env, uint32_t i,
    camspork::StmtRef *out_stmt) {
  CAMSPORK_API_PROLOGUE
...
CAMSPORK_EXPORT int camspork_get_num_remarks(
...
const camspork::extent_t *camspork_syncv_fail_idx_ptr(
    const camspork::ProgramEnv *p_env) {
