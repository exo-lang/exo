...
#define RETURN_MUTATE_TAGGED(name)                                             \
  ...
  static const char *table[] = {name "::Read", name "::Mutate", name "::RAW",  \
      name "::WAR", name "::WAW"};                                             \
  return table[idx];
...
void ExcutSyncEnvAccess::write_args(FILE *file) const {
  fprintf(file, "[\"int:%u\", \"int:%u\", \"str:%s\"", id_before, id_after,
      name.c_str());
  for (extent_t n : idx) {
...
void ExcutTlSigInterval::write_args(FILE *file) const {
  ...
  fprintf(file,
      ...
      "\"int:%u\"]",
      tid_lo, tid_hi, qual_bits_by_vis.array[0], qual_bits_by_vis.array[1],
      qual_bits_by_vis.array[2], qual_bits_by_vis.array[3]);
}
...
const char *ExcutBarrierAlloc::action_name() const { return "barrier_id"; }
