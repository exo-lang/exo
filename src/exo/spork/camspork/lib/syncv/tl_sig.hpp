...
// TODO we are not really using this consistently, maybe -Wconversion can help
// if we need to widen this. However, a better way may be to change Exo to allow
// assigning different bits to stand for different QualTL on a per-program
// basis, as it's unlikely a single program needs more than 32 QualTL.
...
inline const char *vis_flag_index_name(int32_t vis_flag_index) {
  static const char *table[num_vis_flags] = {
      "atomic-only", "temporal", "full", "issue"};
  CAMSPORK_REQUIRE_CMP(size_t(vis_flag_index), <, size_t(num_vis_flags),
      "invalid vis flag index");
...
inline QualBitsByVis qual_vis_product(
    qual_bits_t qual_bits, int32_t vis_flags) {
  QualBitsByVis qv;
...
// A single timeline signature (tl-sig) is the 3-tuple (thread ID, qual-tl,
// visibility flag) We usually don't store this directly. Instead, we work with
// sets of tl-sig. A TlSigInterval encodes the subset of timeline signatures (t,
// q, v) where
//
// tid_lo <= t < tid_hi
...
struct TlSigInterval {
  ...
  void assert_valid() const {
    CAMSPORK_REQUIRE(qual_bits_by_vis.intersects(qual_bits_by_vis),
        "Empty qual_bits_by_vis");
  ...
  bool operator!=(TlSigInterval other) const { return !(*this == other); }
  ...
  bool is_atomic_only() const {
    ...
    static_assert(vis_flag_index_atomic_only == 0,
        "Hard-wired code for skipping atomic-only");
