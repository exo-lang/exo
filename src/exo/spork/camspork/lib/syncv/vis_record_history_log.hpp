...
struct vis_record_version_t {
  ...
  explicit operator bool() const { return _1_index != 0; }
};
...
struct LoggedVisRecordOrigin {
  ...
  LoggedSyncStmtEvent
      sync_stmt_event;  // Unused if previous_version = 0, except thread_cuboid.
  stmt_id_bits_t stmt_id_bits;  // Stmt that led to this VisRecord being created
                                // (access) or changing (sync).
};
...
private:
  ...
  // Internal tables.
  // We store the history of each VisRecord ever recorded, so we have to
  // translate vis_record_id_t (which may be re-used) to vis_record_version_t
  // (which refers to a specific snapshot of a VisRecord independent of any
  ...
  std::vector<LoggedVisRecordData>
      version_data;  // Indexed by [vis_record_version_t::_1_index - 1]
  std::vector<LoggedVisRecordOrigin>
      version_origin;  // Indexed by [vis_record_version_t::_1_index - 1]
  ...
  // added by a read/mutate and then later encountered by a different
  // mutate/read.
...
public:
  ...
  // ******************************************************************************************
  // Callbacks intended for the syncv (synchronization validation)
  // implementation.
  // ******************************************************************************************
  void set_syncv_sync_stmt_info(barrier_id,
      LoggedSyncStmtValues);  // Affects later log_syncv_vis_record_change.
  void log_syncv_new_vis_record(vis_record_id_t id, LoggedVisRecordData data);
  void log_syncv_vis_record_change(vis_record_id_t old_id,
  ...
  void log_syncv_vis_record_error(
  ...
  void add_history_remarks(ProgramEnv *,
      vis_record_version_t last_version);  // For debugging syncv impl
