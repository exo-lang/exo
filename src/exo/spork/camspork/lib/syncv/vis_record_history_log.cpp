...
// NOTE: for the most part, I try to keep this "core" syncv (synchronization
// validation) code separate from the less-than-great program interpreter. To
// decouple this code from the interpreter, remove the "add_remarks" functions.
...
void stream_sync_stmt_event(
    Stream &s, VisRecordHistoryLog &log, const LoggedSyncStmtEvent &event) {
  ...
  if (is_arrive ^ is_await) {
    ...
    s << "  arrive_count: " << v.arrive_count_before << " -> "
      << v.arrive_count_after << '\n';
    s << "  await_count:  " << v.await_count_before << " -> "
      << v.await_count_after << '\n';
    if (is_await) {
      s << "  > Updates pending awaits with arrive_count <= "
        << v.await_max_arrive_count << '\n';
    }
  }
  if (!is_await) {
    s << "  First qual-tl set:\n";
...
template <typename Stream>
void stream_tid(
    Stream &s, uint32_t tid, const ThreadCuboid &cuboid_for_domain) {
  if (tid + 2 <= 1) {
    s << "...";
    return;
  }
...
template <typename Stream>
void stream_vis_record(Stream &s, VisRecordHistoryLog &log,
    const ThreadCuboid &cuboid_for_domain, const LoggedVisRecordData &data,
...
void VisRecordHistoryLog::set_syncv_sync_stmt_info(
    barrier_id bar, LoggedSyncStmtValues values) {
...
void VisRecordHistoryLog::log_syncv_new_vis_record(
    vis_record_id_t id, LoggedVisRecordData data) {
...
void VisRecordHistoryLog::log_syncv_vis_record_change(vis_record_id_t old_id,
    vis_record_id_t new_id, LoggedVisRecordData new_data, bool debug_printf) {
  ...
  if (debug_printf) {
    ...
    stream_vis_record(before, *this, current_thread_cuboid,
        version_data.at(old_version._1_index - 1), true);
    std::stringstream after;
    stream_vis_record(
        after, *this, current_thread_cuboid, version_data.back(), true);
    if (false) {
      printf("[%i] ID %s\n", debug_printf_counter,
          old_id == new_id ? "SAME" : "CHANGED");
    } else {
      printf("[%i] %u -> %u\n", debug_printf_counter, unsigned(old_id),
          unsigned(new_id));
      printf("vis_record_version_t(%llu)\n",
          static_cast<long long unsigned>(version._1_index));
    }
    printf("BEFORE:%s\nAFTER:%s\n", std::move(before).str().c_str(),
        std::move(after).str().c_str());
  }
}

void VisRecordHistoryLog::log_syncv_vis_record_checked(
    vis_record_id_t id, bool is_mutate) {
...
void VisRecordHistoryLog::log_syncv_vis_record_error(
    vis_record_id_t id, LoggedMissingTlSig fail_tl_sig) {
...
void VisRecordHistoryLog::add_error_remarks(ProgramEnv *p_env) {
  ...
  if (error_vis_record_version) {
    std::stringstream s;
    s << "VisRecord did not have vis flag \""
      << vis_flag_index_name(error_tl_sig.vis_flag_index) << "\" for\n";
    s << "  thread:  ";
...
void VisRecordHistoryLog::add_history_remarks(
    ProgramEnv *p_env, vis_record_version_t last_version) {
  ...
  // We search back in time until we found when the VisRecord was originally
  // created and remark on it + all transitions to the present state. We want
  // old state to appear first, so we have to buffer remarks and reverse them.
  ...
  while (tmp_v) {
    ...
    if (origin.previous_version) {
      s << origin.sync_stmt_event.values.sync_stmt_name
        << " recorded, which caused VisRecord change:\n";
      ...
      stream_vis_record(
          s, *this, origin.sync_stmt_event.thread_cuboid, previous_data);
      s << "AFTER:\n";
    } else {
      s << "New VisRecord:\n";
    }
