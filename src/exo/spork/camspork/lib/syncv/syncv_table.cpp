...
struct SyncvTable
{
    ...
    bool synchronizes_with(const VisRecord& vis_record, const ThreadCuboid& cuboid, qual_bits_t qual_bits)
    {
        ...
        cuboid.to_intervals([&] (uint32_t tid_lo, uint32_t tid_hi)
        {
            const qual_bits_t qual_bits_mask =
                    Transitive ? ~qual_bits_t(0) : qual_bits_t(1) << vis_record.original_qual_tl;
            while (true) {
                if (!node_id) {
                    return;  // Exit lambda
                }
                ...
                TlSigInterval gen_interval{tid_lo, tid_hi, {{qual_bits, qual_bits, 0}}};
    ...
    bool visible_to(
            const VisRecord& vis_record, const ThreadCuboid& cuboid, qual_bits_t want_qual_bits, int32_t vis_level)
    {
        ...
        cuboid.to_intervals([&] (uint32_t tid_lo, uint32_t tid_hi)
        {
            ...
            while (node_id) {
                ...
                // a qual-tl in common in at the required visibility level.
                const TlSigIntervalListNode& node = get(node_id);
                node_id = node.camspork_next_id;
                const TlSigInterval data = node.data;
                if (0 == (data.qual_bits_by_vis.array[vis_level] & want_qual_bits)) {
                    continue;
                }
    ...
    // the unordered visibility set (ignoring atomic-only portions). TODO update vocabulary.
    ...
    TlSigBucketKey minimal_superset_interval(nodepool::id<TlSigIntervalListNode> id) const
    {
        ...
        while (id) {
            ...
            id = node.camspork_next_id;
            const TlSigInterval data = node.data;
            if (data.qual_bits_by_vis.array[vis_level_unordered] != 0) {
            ...
            std::vector<pending_await_t> pending_awaits)
    {
        if (transitive) {
            ...
            ArriveUpdateCommand<false> command{&cuboid, L1_qual_bits, std::move(pending_awaits), {}};
            update_vis_records_for_sync_impl(command);
        }
    }
    ...
    {
        augment_counter++;
        update_vis_records_for_fence(transitive, cuboid, L1_qual_bits, L2_full_qual_bits, L2_temporal_qual_bits);
    }
            ...
            bool transitive, const ThreadCuboid& cuboid, uint32_t L1_qual_bits)
    {
        ...
        state.arrive_count++;
        update_vis_records_for_arrive(transitive, cuboid, L1_qual_bits, std::move(pending_awaits));
    }
    ...
    template <bool IsMutate, bool UpdateRecords, typename Input, typename Logger>
            ...
            Logger&& logger)
    {
                ...
                nodepool::id<VisRecordListNode<IsMutate>> new_vis_record_id)
        {
            ...
            if constexpr (IsMutate) {
                ...
                // Add the single new mutate visibility record.
                ...
                if (q_input.atomic_qual_bits != 0) {
                    ...
                    assignment_record.mutate_vis_records_head_id = new_head_id;
...
void on_fence(SyncvTable* table, bool transitive, const ThreadCuboid& cuboid,
        uint32_t L1_qual_bits, qual_bits_t L2_full_qual_bits, qual_bits_t L2_temporal_qual_bits)
{
    INTERFACE_PROLOGUE(table)
    table->on_fence(transitive, cuboid, L1_qual_bits, L2_full_qual_bits, L2_temporal_qual_bits);
    INTERFACE_EPILOGUE(table)
}
...
{
    INTERFACE_PROLOGUE(table)
    table->on_arrive(home_barrier, barrier_count, all_barriers, transitive, cuboid, L1_qual_bits);
    INTERFACE_EPILOGUE(table)
}
