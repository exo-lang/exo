...
using BinaryTree = std::map<K, V>;
template <typename K, typename V>
using Map = std::unordered_map<K, V>;
template <typename V>
using Set = std::unordered_set<V>;
template <typename V>
using MultiSet = std::unordered_multiset<V>;
...
// These used to be reference-counted, but are now uniquely owned (fix outdated
// comments if found).
struct PendingAwaitNode {
  nodepool::id<PendingAwaitNode> camspork_next_id;
  pending_await_t await_id;

  refcnt_t get_refcnt() const { return 1; }
};
...
static_assert(sizeof(TlSigIntervalListNode) == 28,
    "Check that you meant to change this perf-critical struct");
...
struct VisRecordListNode {
  ...
  // If in the forwarding state, this is an owning reference to the forwarded-to
  // visibility record.
  nodepool::id<VisRecordListNode<K>> camspork_next_id;

  uint64_t forwarded_flag : 1;

  // This has nothing to do with the main purpose of the struct; only needed for
  // assignment_record_remove_duplicates.
  ...
  // If the visibility record is in the forwarding state, the data is that of
  // the record at get(camspork_next_id).
  VisRecord base_data;

  bool is_forwarded() const { return forwarded_flag; }

  refcnt_t get_refcnt() const { return refcnt; }
};

using DefaultVisRecordListNode = VisRecordListNode<VisRecordKind::Default>;

static_assert(sizeof(DefaultVisRecordListNode) == 24,
    "Check that you meant to change this perf-critical struct");
...
using DefaultAssignmentRecordVisNode =
    AssignmentRecordVisNode<VisRecordKind::Default>;

// Assignment record: collection of mutate visibility records + collection of
// read visibility records. This is associated for each position (scalar, or
// value in a tensor) of the program undergoing synchronization validation.
//
// Multiple positions may reference the same assignment record.
// We implement a copy-on-write strategy (exception: may modify in-place if no
// one will hold a reference to the old assignment record anymore).
struct AssignmentRecord {
  ...
  // This can become a bitfield if we need the space for something else, but I
  // had issues with -Wconversion.
...
struct BarrierArriveState {
  ...
  // Forwarding-state VisRecords may be in the lists as well ... ignore them if
  // found.
  //
...
using CensusMap =
    Map<nodepool::id<AssignmentRecord>, AssignmentRecordCensusEntry>;
...
struct history_log_vis_record_id {
  ...
  history_log_vis_record_id(nodepool::id<VisRecordListNode<K>> node_id) {
    ...
    // If multiple VisRecordKind are added, then we have to add some bits to
    // disambiguate.
    static_assert(sizeof(data) == 4);
    data = node_id.id_bits;
    // This used to be: data = node_id.id_bits << 1 | IsMutate;
    // CAMSPORK_REQUIRE_CMP(node_id.id_bits, <=, 0x7FFF'FFFF, "too many node
    // IDs");
  }

  operator VisRecordHistoryLog::vis_record_id_t() const { return data; }
};
...
struct SyncvTrivialLogger {
  ...
  void history_set_sync_stmt_info(
      const SyncvArrive &, const BarrierState &, uint32_t) {}

  void history_set_sync_stmt_info(
      const SyncvAwait &, const BarrierState &, uint32_t, uint32_t) {}

  void history_set_sync_stmt_info(const SyncvJoinThreads &) {}

  template <VisRecordKind K>
  void history_new_vis_record(
      SyncvTable &, nodepool::id<VisRecordListNode<K>>) {}

  template <VisRecordKind K>
  void history_vis_record_change(SyncvTable &,
      nodepool::id<VisRecordListNode<K>>, nodepool::id<VisRecordListNode<K>>,
      bool) {}
  ...
  void history_vis_record_error(
      nodepool::id<VisRecordListNode<K>>, LoggedMissingTlSig) {}
};
...
struct VisRecordEntropy {
  ...
  void operator()(pending_await_t await_id) { pcg3d_z(await_id); }
...
// "Everything" struct that implements "backend state" for the synchronization
// and barrier environments. The environments consist of IDs that index into
// this table. The reason we have this is the synchronization enviroment defines
// many "global" operations that potentially modify every visibility record in
// existence, so we centralize their state here, and optimize these global
// operations by memoizing identical visibility records.
//
// NOTE: do NOT include "back pointers" to the SyncvTable as that will defeat
// copying. For the most part this is trivially copyable because of our use of
// node pools. A linked list can be copied by just copying the node pool -- the
// IDs (indexing into the pools) remain valid for the copy.
struct SyncvTable {
  // Failure flag
  // This is to be set upon an exception being thrown through the non-private
  // interface. If this happens, the internal state may be inconsistent, but
  // memory shouldn't be formally leaked since we'll delete the memory pools
  // later anyway. Ignore all further SyncvTable commands if failed = true.
  bool failed = false;

  // Number of times begin_no_checking was called minus end_no_checking. (TODO
  // remove???)
  ...
  auto get_augment_counter_bits() const {
    return static_cast<
        decltype(AssignmentRecord::lazy_last_augment_counter_bits)>(
        augment_counter);
  }
  ...
  std::tuple<nodepool::Pool<AssignmentRecord>,
      nodepool::Pool<TlSigIntervalListNode>, nodepool::Pool<PendingAwaitNode>,
      nodepool::Pool<DefaultVisRecordListNode>,
      nodepool::Pool<DefaultAssignmentRecordVisNode>>
      pool_tuple;
  ...
  //
  // All non-forwarding-state VisRecord must be in the memoization
  // vis_record_table, except that when we apply modifications to the VisRecord
  // objects in the table, the modified ones are temporarily moved into the
  // modified_vis_records list.
  //
  // The VisRecord objects are sorted by hash key. This is split into "chunks"
  // (to avoid expensive insertions), with the full sorted table being the
  // concatenation of all the chunks.
  //
  ...
  // The modified_vis_records table DOES own VisRecord references, to avoid
  // unexpected deallocations causing inconsistent memoization state while we
  // are mapping over the memoization table.
  ...
  template <typename ListNode>
  void insert_next_node(nodepool::id<ListNode> *p_insert_after,
      nodepool::id<ListNode> insert_me) {
    using TypedPool = nodepool::Pool<ListNode>;
    TypedPool &pool = std::get<TypedPool>(pool_tuple);
  ...
  template <typename ListNode>
  [[nodiscard]] nodepool::id<ListNode> remove_next_node(
      nodepool::id<ListNode> *p_id) {
    using TypedPool = nodepool::Pool<ListNode>;
    TypedPool &pool = std::get<TypedPool>(pool_tuple);
  ...
  void remove_and_free_next_node(nodepool::id<ListNode> *p_id) {
    ...
    CAMSPORK_REQUIRE(
        !get(victim_id).camspork_next_id, "Should have been removed from list");
    extend_free_list(
        victim_id);  // so this free only adds 1 node to free chain.
  }
  ...
  void incref(nodepool::id<AssignmentRecord> id) {
    ...
    CAMSPORK_REQUIRE_CMP(
        node.refcnt, !=, 0, "should not have started with 0 refcnt");
    node.refcnt++;
  ...
  void decref(nodepool::id<AssignmentRecord> id, uint32_t nref = 1) {
    ...
    AssignmentRecord &node = get(id);
    CAMSPORK_REQUIRE_CMP(
        nref, <=, node.refcnt, "tried to decref more than the refcnt");
    node.refcnt -= nref;
    if (0 == node.refcnt) {
      reset_assignment_record(&node);
      CAMSPORK_REQUIRE(!node.camspork_next_id,
          "Unexpected: AssignmentRecord next only used for free list");
      extend_free_list(id);
    }
  }
  ...
  template <VisRecordKind K>
  void incref(
      nodepool::id<VisRecordListNode<K>> id, uint32_t added_refcnt = 1) {
    VisRecordListNode<K> &node = get(id);
    CAMSPORK_REQUIRE_CMP(
        node.refcnt, !=, 0, "should not have started with 0 refcnt");
    node.refcnt += added_refcnt;
    CAMSPORK_REQUIRE_CMP(
        node.refcnt, >, added_refcnt, "reference count overflow");
  }
  ...
  void decref(nodepool::id<VisRecordListNode<K>> id) {
    ...
    if (0 == --node.refcnt) {
      if (node.is_forwarded()) {
        ...
        CAMSPORK_REQUIRE(
            fwd_id, "reporting forwarding state but not forwarded anywhere");
        free_single_vis_record(id);
        decref(fwd_id);  // Hope for tail call.
      } else {
        // Non-forwarded (base) visibility record must be removed from
        // memoization first.
        auto memoized_id = remove_memoized(id);
        CAMSPORK_REQUIRE_CMP(
            id, ==, memoized_id, "should have been found in memoization table");
        CAMSPORK_REQUIRE(!get(memoized_id).camspork_next_id,
            "Should not have forwarding id set.");
  ...
  template <VisRecordKind K>
  void assignment_record_remove_vis_records(
      nodepool::id<AssignmentRecordVisNode<K>> *p_head_id) {
  ...
  // If the visibility record is modified, you need to be careful to update the
  // memoization table.
  ...
  VisRecordListNode<K> &alloc_vis_record(const ThreadInit &thread_init,
      SyncvAccessInfo access, nodepool::id<VisRecordListNode<K>> *out) {
    nodepool::id<VisRecordListNode<K>> vis_record_id;
    ...
    // Initialize visibility set = linked list of intervals generated from the
    // initial thread / thread cuboid.
    ...
    nodepool::id<TlSigIntervalListNode> *p_node_id =
        &vis_record.base_data.visibility_set;
    ...
    CAMSPORK_REQUIRE(
        vis_record.base_data.visibility_set, "buggy: empty visibility set");
    ...
    for (uint32_t i = 0; i < access.barrier_count; ++i) {
      ...
      const pending_await_t info =
          pack_pending_await(barrier_index, state.arrive_count);
      ...
      // forwarding-state VisRecord, which isn't a well-tested code path
      // probably.
    ...
    if (access.atomic_qual_bits) {
      ...
      const QualBitsByVis qual_bits_by_vis =
          qual_vis_product(access.atomic_qual_bits, vis_flag_atomic_only);
  ...
  void union_tl_sig_interval(VisRecord *p, TlSigInterval input) {
    // Non-empty input check (cartesian product of non-empty thread interval and
    // non-empty qual-tl set).
    CAMSPORK_REQUIRE_CMP(
        input.tid_hi, >, input.tid_lo, "non-empty input check");
    {
    ...
    // We can view each pointer-to-node_id as a "gap" between intervals (imagine
    // an arrow between nodes = a gap). We abuse language to consider there to
    // be a "gap" before the leftmost and after the rightmost intervals. This
    // runs for N+1 iterations where N is the current interval count.
    uint32_t gap_tid_lo = 0;
    for (node_id *p_id = &p->visibility_set; 1;) {
      // Must remember the next iteration's node now, so we don't get confused
      // by insertion.
      ...
      if (original_next_node_id) {
        ...
        CAMSPORK_REQUIRE_CMP(
            gap_tid_lo, <=, gap_tid_hi, "intervals were out of order.");
      } else {
        gap_tid_hi = input.tid_hi;  // For "gap" after the rightmost interval.
      }

      // If the gap is non-empty and overlaps the input interval, we need to
      ...
      // This runs N times (leftmost interval is the "next node" of the
      // imaginary before-left gap) We will modify each original interval, which
      // is the "next node" of the gap just processed.
      //
      // The interval may be subdivided into up to 3 intervals depending on
      // overlap with input interval, since only the overlapped portion should
      // have its qual_bits_by_vis modified.
      //
      ...
      const uint32_t intersect_tid_lo =
          std::max(original_data.tid_lo, input.tid_lo);
      const uint32_t intersect_tid_hi =
          std::min(original_data.tid_hi, input.tid_hi);
      const QualBitsByVis union_bits =
          input.qual_bits_by_vis | original_data.qual_bits_by_vis;
      const bool change_needed =
          (union_bits != original_data.qual_bits_by_vis) &&
          (intersect_tid_lo < intersect_tid_hi);
      ...
      if (change_needed && original_data.tid_lo < intersect_tid_lo) {
        ...
        CAMSPORK_REQUIRE_CMP(
            *p_id, ==, original_next_node_id, "linked list corrupt");
        insert_next_node(p_id, new_node_id);
      }

      // Now update iteration state (we do this now so the following insertions
      // work)
      p_id = &next_node.camspork_next_id;
      gap_tid_lo = next_node.data.tid_hi;

      if (change_needed) {
        // 2nd interval; we recycle the existing node since the 2nd interval is
        ...
        if (intersect_tid_hi < original_data.tid_hi) {
          ...
          new_node.data.qual_bits_by_vis = original_data.qual_bits_by_vis;
          insert_next_node(p_id, new_node_id);
          p_id = &new_node.camspork_next_id;  // Need to point to the real gap
                                              // (after original_data.tid_hi)
        }
      }
    }

    // Merge redundant intervals
    // For each "current node", we try to merge it with the next node, if it
    // exists.
    ...
    for (node_id next_id; (next_id = p_current_node->camspork_next_id);) {
      ...
      CAMSPORK_REQUIRE_CMP(
          current.tid_lo, <, current.tid_hi, "invalid interval");
      CAMSPORK_REQUIRE_CMP(
          current.tid_hi, <=, next.tid_lo, "invalid interval overlap");
      CAMSPORK_REQUIRE_CMP(next.tid_lo, <, next.tid_hi, "invalid interval");

      if (current.tid_hi == next.tid_lo &&
          current.qual_bits_by_vis == next.qual_bits_by_vis) {
        // Merge next node into current node, and remove next node from list.
        current.tid_hi = next.tid_hi;
        CAMSPORK_REQUIRE_CMP(p_current_node->camspork_next_id, ==, next_id,
            "corrupt linked list");
        remove_and_free_next_node(&p_current_node->camspork_next_id);
      } else {
        // If not merged, we need to process the next node in the next
        // iteration. If we did merge, we keep the same current node, since we
        // may need to merge with the new next node.
  ...
  bool valid_adjacent(TlSigInterval first, TlSigInterval second) const {
    // Check that the two tl_sig intervals can be adjacent in a valid visibility
    // set encoding.
    return first.tid_lo < first.tid_hi && first.tid_hi <= second.tid_lo &&
           second.tid_lo < second.tid_hi &&
           (first.tid_hi < second.tid_lo ||
               first.qual_bits_by_vis != second.qual_bits_by_vis);
  }
  ...
  bool equal(const VisRecord &a, const VisRecord &b) const {
    ...
    // Check equal intervals. We rely on (and enforce) the non-redundant
    // encoding requirement.
    {
      ...
      while (id_a && id_b) {
        ...
        CAMSPORK_REQUIRE(
            !id_a || valid_adjacent(current_a.data, get(id_a).data),
            "invalid adjacent intervals");
        CAMSPORK_REQUIRE(
            !id_b || valid_adjacent(current_b.data, get(id_b).data),
            "invalid adjacent intervals");
  ...
  bool synchronizes_with(const VisRecord &vis_record,
      const ThreadCuboid &cuboid, qual_bits_t qual_bits) {
    // Check for any intersections between TlSigIntervals generated by
    // ThreadCuboid + qual_bits + flags, and those stored in the VisRecord, with
    // flags vis_flag_issue or vis_flag_full.
    bool intersects = false;
    nodepool::id<TlSigIntervalListNode> node_id = vis_record.visibility_set;
    const QualBitsByVis qv =
        qual_vis_product(qual_bits, vis_flag_issue | vis_flag_full);
    cuboid.to_intervals([&](uint32_t tid_lo, uint32_t tid_hi) {
      while (true) {
        ...
        if (vis_set_interval.tid_hi > tid_hi) {
          // Keep checking new vis_set_interval against gen_interval until the
          // visibility set interval has a tid_hi above that of the generated
          // interval (from the ThreadCuboid). NB on this code path node_id is
          // not updated, so vis_set_interval will be checked against a new
          // gen_interval.
          return;
        }
  ...
  bool all_visible_to(const VisRecord &vis_record, const ThreadCuboid &cuboid,
      qual_bits_t want_qual_bits, int32_t vis_flag,
      LoggedMissingTlSig *out_fail_tl_sig) {
    // IsConvergent = false case
    // Check that all threads generated by the ThreadCuboid have at least one
    // qual-tl in common with an overlapping interval in the ordered visibility
    // set of the vis_record.
    ...
    cuboid.to_intervals([&](uint32_t tid_lo, uint32_t tid_hi) {
      nodepool::id<TlSigIntervalListNode> node_id = vis_record.visibility_set;
      // Interval of threads [missing_threads_lo, tid_hi) not yet found to
      // overlap with a valid TlSigInterval in the visibility set.
      ...
      while (node_id) {
        ...
        // If the interval is after the interval of missing threads, we have
        // found a gap [missing_threads_lo, data.tid_lo) that is not found. We
        // fail at this point (break out of loop).
      ...
      if (!local_visible && all_visible) {
        ...
        out_fail_tl_sig->qual_bits = want_qual_bits;
        out_fail_tl_sig->vis_flag_index = vis_flag_index;
      }
    });
    return all_visible;
  }

  bool any_visible_to(const VisRecord &vis_record, const ThreadCuboid &cuboid,
      qual_bits_t want_qual_bits, int32_t vis_flag,
      LoggedMissingTlSig *out_fail_tl_sig) {
    // IsConvergent = true case
    // Check that at least one thread generated by the ThreadCuboid has at least
    // one one qual-tl in common with an overlapping interval in the ordered
    // visibility set of the vis_record.
    const int vis_flag_index = get_low_bit_index(vis_flag);
    CAMSPORK_REQUIRE_CMP(vis_flag, ==, (1 << vis_flag_index), "not a vis flag");
    CAMSPORK_REQUIRE_CMP(vis_flag_index, <, num_vis_flags, "not a vis flag");
  ...
  bool visible_to(const VisRecord &vis_record, const ThreadCuboid &cuboid,
      qual_bits_t want_qual_bits, int32_t vis_flag, bool is_convergent,
      LoggedMissingTlSig *out_fail_tl_sig) {
    if (is_convergent) {
      return any_visible_to(
          vis_record, cuboid, want_qual_bits, vis_flag, out_fail_tl_sig);
    } else {
      return all_visible_to(
          vis_record, cuboid, want_qual_bits, vis_flag, out_fail_tl_sig);
    }
  }

  // *** Barrier ID Allocation ***
  // For now barrier_id::data only stores the barrier index number + 1, but this
  // could change.
  ...
  uint32_t get_barrier_index(barrier_id bar) const {
    ...
    CAMSPORK_REQUIRE_CMP(
        index, <, max_live_barriers, "max_live_barriers limit exceeded");
  ...
  void alloc_barriers(size_t N, barrier_id *barriers) {
    ...
    for (uint32_t word_index = 0; word_index < max_live_barriers / 64;
         ++word_index) {
      uint64_t negated_bits;
      while ((negated_bits = ~live_barrier_bits[word_index]) != 0) {
        CAMSPORK_REQUIRE_CMP(barriers[num_allocated].data, ==, 0,
            "allocated barrier without free");
    ...
    CAMSPORK_REQUIRE(false,
        "Exceeded implementation limit (max number of barriers per program)");
  }
  ...
  void free_barriers(size_t N, barrier_id *barriers, bool check_arrive_await) {
    for (size_t i = 0; i < N; ++i) {
      ...
      if (check_arrive_await) {
        if (state.arrive_count != state.await_count) {
          std::string message =
              "Arrive count (" + std::to_string(state.arrive_count) +
              ") != Await count (" + std::to_string(state.await_count) + ")";
          throw SyncvCheckFail{std::move(message), i};
        }
      }

      // Normally, retire_barrier_arrive augments VisRecords, but here we just
      // pass no_op, so the only intended effect is for us to free memory.
      for (auto &pair : state.arrive_states) {
        ...
        const pending_await_t await_id =
            pack_pending_await(barrier_index, arrive_count);
    ...
    memoize_modified(SyncvTrivialLogger{});
  }
  ...
  uint64_t hash_vis_record(const VisRecord &vis_record) const {
    // 62-bit hash
    // (hi)
    // 32 bits: max tid of timeline signatures (tid, q, v) with v !=
    // atomic-only; 0 if no such tid. 30 bits: entropy (lo)
    //
    static_assert(VisRecord::num_memoize_hash_bits == 62);
    uint32_t non_atomic_max_tid = 0;
    VisRecordEntropy entropy{};

    const nodepool::id<TlSigIntervalListNode> *p_id =
        &vis_record.visibility_set;
    ...
    while (*p_id) {
      ...
      non_atomic_max_tid = std::max(non_atomic_max_tid,
          tl_sigs_atomic_only ? uint32_t(0) : tl_sigs.tid_hi - 1u);
  ...
  // could theoretically be modified when interpreting an Arrive with the
  // minimum tid_lo in the ThreadCuboid being min_tid_lo.
  static std::pair<uint64_t, uint64_t> hash_bounds_for_arrive(
      uint32_t min_tid_lo) {
  ...
  enum class MemoizeAction {
    ...
    // If matching VisRecord found, remove it from table and return its id; else
    // return 0.
    Remove,

    // If matching VisRecord found, set the command VisRecord to the forwarding
    // state pointing to the matching VisRecord. Return the ID of the matching
    // VisRecord in the memoization table. Otherwise, add the command VisRecord
    // to the memoization table. Recall that the memoization table does not own
    // its reference to the VisRecord.
    MemoizeOrForward,

    // Run callback for all memoized VisRecords with hash values in the
    // inclusive hash_bounds range. Any modified VisRecord are removed from the
    // table and added to modified_vis_records. Must call memoize_modified()
    // afterwards.
    EditAll,
  ...
  // Skeleton function for a variety of functions interacting with the
  // memoization table.
  //
  ...
  //     // The hash of the VisRecord must be in the inclusive bounds of
  //     hash_bounds. nodepool::id<VisRecordListNode<K>> node_id;
  //
  //     bool operator() (SyncvTable& env, nodepool::id<VisRecordListNode<K>>
  //     vis_record_id)
  //         // needed for EditOne and EditAll.
  //         // Returns `changed` flag.
  template <typename Command>
  nodepool::id<VisRecordListNode<VisRecordKind::Default>>
  for_vis_record_hash_bounds(
      std::pair<uint64_t, uint64_t> hash_bounds, Command &&command) {
    ...
    constexpr MemoizeAction memoize_action =
        std::remove_reference_t<Command &&>::memoize_action;
    ...
    if constexpr (memoize_action != MemoizeAction::EditAll) {
      ...
      CAMSPORK_REQUIRE(!p_command_node->is_forwarded(),
          "command input node must not be forwarded");
    ...
    const auto first_chunk_iter = std::lower_bound(vis_record_table.begin(),
        vis_record_table.end(), VisRecordChunkMaxHash{hash_lo});
    ...
    if (first_chunk_iter != vis_record_table.end()) {
      ...
      // Find the index within the chunk of a VisRecord having a hash at least
      // hash_lo.
      intra_chunk_index =
          static_cast<size_t>(std::lower_bound(first_chunk_iter->nodes.begin(),
                                  first_chunk_iter->nodes.end(), hash_lo,
                                  [this](auto lhs, auto rhs) {
                                    return this->read_hash_helper(lhs) <
                                           this->read_hash_helper(rhs);
                                  }) -
    ...
    for (; chunk_index < vis_record_table.size(); chunk_index++) {
      ...
      for (; intra_chunk_index < chunk.nodes.size(); intra_chunk_index++) {
        nodepool::id<VisRecordListNode<K>> cur_id =
            chunk.nodes[intra_chunk_index];
        const VisRecordListNode<K> &cur_node = get(cur_id);
        CAMSPORK_REQUIRE(!cur_node.is_forwarded(),
            "Forwarded VisRecord should not be memoized?");

        CAMSPORK_REQUIRE_CMP(debug_hash, <=, cur_node.memoize_hash_bits,
            "VisRecord sorting bug");
        ...
        if (cur_node.memoize_hash_bits > hash_hi) {
          if constexpr (memoize_action == MemoizeAction::MemoizeOrForward) {
            chunk.nodes.insert(
                chunk.nodes.begin() + intra_chunk_index, command.node_id);
            ...
            if (chunk.nodes.size() > max_chunk_size) {
              ...
              vis_record_table.insert(
                  vis_record_table.begin() + chunk_index + 1,
                  std::move(new_chunk));
        ...
        if constexpr (memoize_action == MemoizeAction::Find) {
        ...
        } else if constexpr (memoize_action ==
                             MemoizeAction::MemoizeOrForward) {
          ...
          if (equal(command_node.base_data, cur_node.base_data)) {
            ...
            CAMSPORK_REQUIRE_CMP(fwd_id, !=, command.node_id,
                "Trying to memoize something already in the memoization "
                "table.");

            reset_vis_record_data(&command_node.base_data);
            command_node.camspork_next_id = fwd_id;
            command_node.forwarded_flag = 1;
            CAMSPORK_REQUIRE(command_node.is_forwarded(),
                "should now be in forwarding state");
          ...
          if constexpr (memoize_action == MemoizeAction::EditOne) {
            if (equal(p_command_node->base_data, cur_node.base_data)) {
              CAMSPORK_REQUIRE_CMP(
                  command.node_id, ==, cur_id, "Non-unique entry somehow?");
            } else {
              should_edit = false;
            }
          }
          ...
          if (should_edit) {
            ...
            // Otherwise, this could cause the memoization table to change
            // unexpecedly.
            incref(cur_id);
      ...
      // Inspect all chunks starting from index 0 except for the first one
      // inspected which the second std::lower_bound allowed us to skip some
      // work in.
      intra_chunk_index = 0;
  ...
  uint64_t read_hash_helper(uint64_t hash) const { return hash; }
  ...
  template <VisRecordKind K>
  VisRecordListNode<K> &remove_forwarding_node(
      nodepool::id<VisRecordListNode<K>> *p_id) {
    ...
    incref(id);
    decref(old_id);  // Will take care of deallocating chain of forwarding if
                     // needed.
  ...
  // Like remove_forwarding but non-destructive, i.e., don't actually replace
  // the ID of a forwarding visibility record with that of the forwarded-to base
  // visibility record.
  template <VisRecordKind K>
  VisRecord const_resolve_forwarding(nodepool::id<VisRecordListNode<K>> id,
  ...
  // Add a new visibility record, or return existing memoized one, constructed
  // from the given thread cuboid
  // + qual_bits_by_vis (in TlSigInterval format).
  // The returned ID is an owning reference (ownership count given by
  // added_refcnt).
  template <VisRecordKind K, typename ThreadInit>
  [[nodiscard]] nodepool::id<VisRecordListNode<K>> memoize_new_vis_record(
      const ThreadInit &thread_init, SyncvAccessInfo access,
    ...
    if (!new_vis.base_data.pending_awaits) {
      CAMSPORK_REQUIRE_CMP(
          new_vis.refcnt, ==, 1, "expected 1 refcnt initially");
    }

    // Either insert into memoization, or forward to existing duplicate.
    // result_id gains added_refcnt-many references, while the originally
    // created VisRecord (which may be the same one, if not a duplicate) loses
    // the 1 refcnt that it was initially created with.
    //
    // In most cases, for a duplicate, the decref leads to the duplicate being
    // deleted. However, this is not the case if alloc_vis_record caused the
    // VisRecord to gain additional references (due to pending awaits). It would
    // be more efficient (but riskier) to defer adding those pending await
    // references until after we know this is not a duplicate, so we can free
    // instantly.
  ...
  // This removes the given node from the memoization table, but does not
  // decrement the reference count or free it. Recall that the memoization table
  // does not own (reference count) the VisRecords contained.
  template <VisRecordKind K>
  ...
  template <VisRecordKind K>
  nodepool::id<VisRecordListNode<K>> find_memoized(
      nodepool::id<VisRecordListNode<K>> id) const {
    CAMSPORK_REQUIRE(id, "unexpected null");

    FindMemoizedCommand<K> command{id};
    const uint64_t hash = read_hash_helper(id);
    return const_cast<SyncvTable *>(this)->for_vis_record_hash_bounds(
        {hash, hash}, command);
  }
  ...
  // Given an existing visibility record in the base state that's not in the
  // memoization table, either
  //   * Add it to the memoization table, if it's unique. Return itself.
  //   * Put it in the forwarding state (discard existing state) and forward to
  //   equal already-memoized record.
  //     Return ID of memoized record.
  // This initializes VisRecordListNode::memoize_hash_bits, as specified by the
  // documentation for that data member.
  template <VisRecordKind K>
  nodepool::id<VisRecordListNode<K>> memoize_or_forward(
      nodepool::id<VisRecordListNode<K>> id) {
    CAMSPORK_REQUIRE(id, "unexpected null");
    VisRecordListNode<K> &node = get(id);
    ...
    CAMSPORK_REQUIRE(
        !node.camspork_next_id, "shouldn't be in any linked list (forwarded?)");

    // Calculate hash here as promised in the comment for the memoize_hash_bits
    // member variable.
    const auto hash = hash_vis_record(node.base_data);
    node.memoize_hash_bits =
        hash;  // No way to silence stupid bitfield truncation warning!
  ...
  void memoize_modified(Logger &&logger) {
    for (nodepool::id<VisRecordListNode<VisRecordKind::Default>> id :
        modified_vis_records) {
      const auto fwd_id = memoize_or_forward(id);
      logger.history_vis_record_change(*this, id, fwd_id, false);
      decref(id);
    }
    modified_vis_records.clear();
  }

  static QualBitsByVis from_L2(
  ...
  struct AugmentVisRecordCallback {
    ...
    template <VisRecordKind K>
    void operator()(
        SyncvTable &env, nodepool::id<VisRecordListNode<K>> vis_record_id) {
      auto &node = env.get(vis_record_id);
      CAMSPORK_REQUIRE(!node.is_forwarded(),
          "Unexpected modification of forwarding state VisRecord");
      p_cuboid->to_intervals([&](uint32_t tid_lo, uint32_t tid_hi) {
        env.union_tl_sig_interval(
            &node.base_data, TlSigInterval{tid_lo, tid_hi, q_by_vis});
      });
    }
  };
  ...
  struct FenceUpdateCommand {
    ...
    template <VisRecordKind K>
    bool operator()(SyncvTable &env,
        nodepool::id<VisRecordListNode<K>> vis_record_id) const {
      auto &node = env.get(vis_record_id);
      ...
      const bool syncs =
          env.synchronizes_with(*p_record, *p_cuboid, L1_qual_bits);
      if (syncs) {
  ...
  const BarrierArriveState *get_const_barrier_arrive_state(
      pending_await_t info) const {
    const auto barrier_index = pending_await_barrier_index(info);
    const auto arrive_count = pending_await_arrive_count(info);
  ...
  struct ArriveUpdateCommand {
    ...
    bool operator()(
        SyncvTable &env, nodepool::id<VisRecordListNode<K>> vis_record_id) {
      CAMSPORK_REQUIRE_CMP(
          L1_qual_bits, !=, 0, "should be if'd out in this case");
      auto &node = env.get(vis_record_id);
      CAMSPORK_REQUIRE(!node.is_forwarded(), "unexpected forwarding state");
      VisRecord *p_record = &node.base_data;
      ...
      if (syncs) {
        ...
        const nodepool::id<PendingAwaitNode> old_await_node =
            p_record->pending_awaits;
        nodepool::id<PendingAwaitNode> new_await_node = old_await_node;
        // Add nodes to the head of the list.
        for (auto iter = pending_awaits.rbegin(); iter != pending_awaits.rend();
             ++iter) {
  ...
  template <typename Logger>
  struct JoinThreadsUpdateCommand {
    static constexpr MemoizeAction memoize_action = MemoizeAction::EditAll;
    static constexpr qual_bits_t L1_qual_bits =
        0;  // Make update_vis_records_for_sync_impl compile...
    ...
    bool operator()(SyncvTable &env,
      ...
      // Check for any intersections between threads generated by ThreadCuboid
      // and those those stored in the VisRecord; union QualBitsByVis of such
      // VisRecord intervals.
      nodepool::id<TlSigIntervalListNode> node_id = p_record->visibility_set;
      p_cuboid->to_intervals([&](uint32_t tid_lo, uint32_t tid_hi) {
        while (true) {
          if (!node_id) {
            return;  // Exit lambda
          }
          const TlSigIntervalListNode &node = env.get(node_id);
          TlSigInterval vis_set_interval = node.data;
          if (vis_set_interval.intersects_threads(tid_lo, tid_hi)) {
            any_change = true;
            q_by_vis |= vis_set_interval.qual_bits_by_vis;
          }
          if (vis_set_interval.tid_hi > tid_hi) {
            // Keep checking new vis_set_interval against gen_interval until the
            // visibility set interval has a tid_hi above that of the generated
  ...
  void extend_barrier_arrive_state(
      nodepool::id<VisRecordListNode<K>> vis_record_id, pending_await_t info) {
  ...
  void retire_barrier_arrive_impl(
      nodepool::id<VisRecordListNode<K>> vis_record_id,
      pending_await_t await_info, Callback &&callback, Logger &&) {
    callback(*this, vis_record_id);

    // Remove PendingAwaitNode.
    nodepool::id<PendingAwaitNode> *p_await_node =
        &get(vis_record_id).base_data.pending_awaits;
    bool found = false;
    while (*p_await_node) {
  ...
  // Find all VisRecords referenced by the BarrierArriveState and remove
  // corresponding pending awaits, then clear the BarrierArriveState. We run the
  // supplied callback (likely AugmentVisRecordCallback) to modify each
  // base-state VisRecord.
  template <typename Callback, typename Logger>
  void retire_barrier_arrive(BarrierArriveState *p_state,
      pending_await_t await_info, Callback &&callback, Logger &&logger) {
    auto retire_list = [&](auto record_id) {
      constexpr VisRecordKind K =
          decltype(record_id)::value_type::vis_record_kind;
      while (record_id) {
        AssignmentRecordVisNode<K> &record_node = get(record_id);
        record_id = record_node.camspork_next_id;
        const nodepool::id<VisRecordListNode<K>> vis_record_id =
            record_node.vis_record_id;
        VisRecordListNode<K> &vis_node = get(vis_record_id);
        nodepool::id<PendingAwaitNode> *p_await_node =
            &vis_node.base_data.pending_awaits;
        if (vis_node.is_forwarded()) {
          CAMSPORK_REQUIRE(!*p_await_node,
              "pending_awaits should be empty for forwarded VisRecord");
        } else {
          const uint64_t hash = read_hash_helper(vis_record_id);
          RetireBarrierArriveCommand<K, Callback &, Logger &> command{
              vis_record_id, await_info, callback, logger};
          const auto modified_id =
              for_vis_record_hash_bounds({hash, hash}, command);
          CAMSPORK_REQUIRE_CMP(
              modified_id, ==, vis_record_id, "Internal error, EditOne failed");
          memoize_modified(logger);
        }
        decref(vis_record_id);
        record_node
            .vis_record_id = {};  // to make things clearer in the debugger.
      }
    };

    nodepool::id<DefaultAssignmentRecordVisNode> &head_id =
        p_state->vis_records_head_id;
    retire_list(head_id);
    extend_free_list(head_id);
    head_id = {};
  }

  // Big payoff for all this code: function that performs the effects of a
  // synchronization statement with the given sync type and given first/second
  // visibility sets. This affects all visibility records whose visibility set
  // intersects with the first visibility set of the synchronization statement.
  //
  // The real entrypoints are the ones specialized for fence, arrive, join
  // threads. Await works differently, using another code path.
  template <typename Command>
  ...
  // Augment all visibility records that synchronize with the first visibility
  // set of the fence.
  template <typename Logger>
  void update_vis_records_for_fence(
      const ThreadCuboid &cuboid, const SyncvFence &fence, Logger &logger) {
    FenceUpdateCommand<Logger> command{&cuboid, fence.L1_qual_bits,
        fence.L2_full_qual_bits, fence.L2_temporal_qual_bits, logger};
    update_vis_records_for_sync_impl(command);
  }

  // Save await_id into all visibility records that synchronize with the first
  // visibility set of the arrive.
  template <typename Logger>
  void update_vis_records_for_arrive(const ThreadCuboid &cuboid,
      qual_bits_t L1_qual_bits, std::vector<pending_await_t> pending_awaits,
      Logger &logger) {
    if (L1_qual_bits == 0) {
      // Do nothing.
    } else {
  ...
  template <typename Logger>
  void update_vis_records_for_await(uint32_t barrier_index,
      int32_t max_arrive_count, const ThreadCuboid &cuboid,
      qual_bits_t L2_full_qual_bits, qual_bits_t L2_temporal_qual_bits,
      Logger &logger) {
    AugmentVisRecordCallback augment{};
    ...
    BinaryTree<int32_t, BarrierArriveState> &arrive_map =
        barrier_states[barrier_index].arrive_states;
    for (auto iter = arrive_map.begin(); iter != arrive_map.end();) {
      ...
      retire_barrier_arrive(&pair.second,
          pack_pending_await(barrier_index, arrive_count), augment, logger);
      arrive_map.erase(iter++);
    }
  }

  template <typename Logger>
  void update_vis_records_for_join_threads(
      const ThreadCuboid &cuboid, const SyncvJoinThreads &, Logger &logger) {
    JoinThreadsUpdateCommand<Logger> command{&cuboid, logger};
    update_vis_records_for_sync_impl(command);
  }
  ...
  template <typename Logger>
  void on_arrive(
      const ThreadCuboid &cuboid, const SyncvArrive &arrive, Logger &&logger) {
    // NB augment_counter not changed, as Arrive does not augment any
    // VisRecords.
    ...
    for (uint32_t i = 0; i < count; ++i) {
      pending_awaits[i] = pack_pending_await(
          get_barrier_index(arrive.all_barriers[i]), state.arrive_count);
    }
  ...
  template <typename Logger>
  void on_await(
      const ThreadCuboid &cuboid, const SyncvAwait &await, Logger &&logger) {
    ...
    logger.history_set_sync_stmt_info(
        await, state, new_await_count, max_arrive_count);
    state.await_count = new_await_count;

    update_vis_records_for_await(barrier_index, max_arrive_count, cuboid,
        await.L2_full_qual_bits, await.L2_temporal_qual_bits, logger);
    // memoize_modified inside retire_barrier_arrive.
  }

  template <typename Logger>
  void on_join_threads(const ThreadCuboid &cuboid,
      const SyncvJoinThreads &join_threads, Logger &&logger) {
    augment_counter++;
    logger.history_set_sync_stmt_info(join_threads);
    update_vis_records_for_join_threads(cuboid, join_threads, logger);
    memoize_modified(logger);
  }
  ...
  static uint32_t assignment_record_window_size(AssignmentRecordWindow window) {
    uint32_t prod = 1;
    for (const uint32_t *p = window.begin_inner_extent;
         p != window.end_inner_extent; ++p) {
      prod *= *p;
    }
    return prod;
  }
  ...
  // Copy one of the linked lists of VisRecord references in an
  // AssignmentRecord.
  template <VisRecordKind K>
  nodepool::id<AssignmentRecordVisNode<K>> copy(
      nodepool::id<AssignmentRecordVisNode<K>> input_id) {
  ...
  AssignmentRecord &copy(const AssignmentRecord &old, uint32_t refcnt,
      nodepool::id<AssignmentRecord> *out_id) {
    ...
    assignment_record.lazy_last_augment_counter_bits =
        old.lazy_last_augment_counter_bits;
    assignment_record.mutate_vis_records_head_id =
        copy(old.mutate_vis_records_head_id);
    assignment_record.read_vis_records_head_id =
        copy(old.read_vis_records_head_id);
    return assignment_record;
  }

  template <bool IsMutate, bool SharedVisRecord, bool UpdateRecords,
      typename Input, typename Logger>
  void checked_on_access_impl(bool is_convergent, Input input,
      const ThreadCuboid &cuboid, SyncvAccessInfo access, Logger &logger) {
    using node_id = nodepool::id<AssignmentRecord>;
    static constexpr VisRecordKind K = VisRecordKind::Default;

    // If the input is a window, take a census of all assignment record IDs in
    // the input window.
    static constexpr bool IsWindow =
        std::is_same_v<decltype(input), AssignmentRecordWindow>;
    using TrivialCensus =
        std::array<std::pair<node_id, AssignmentRecordCensusEntry>, 1>;
    std::conditional_t<IsWindow, CensusMap, TrivialCensus> census;

    if constexpr (IsWindow) {
      cuboid_to_intervals<size_t>(input.begin_outer_extent,
          input.end_outer_extent, input.begin_offset, input.end_offset,
      ...
      census[0].first = node_id{
          input->node_id};  // Where decltype(input) is assignment_record_id*
    ...
    using VisRecordList =
        std::conditional_t<!UpdateRecords, std::array<VisRecordID, 0>,
            std::conditional_t<SharedVisRecord, std::array<VisRecordID, 1>,
                std::vector<VisRecordID>>>;
    ...
    if constexpr (!UpdateRecords) {
      ...
      const VisRecordID new_vis_record_id =
          memoize_new_vis_record<K>(cuboid, access, vis_record_refcnt);
      logger.history_new_vis_record(*this, new_vis_record_id);
      new_vis_record_list[0] = new_vis_record_id;
    } else {
      cuboid.to_intervals([&](uint32_t tid_lo, uint32_t tid_hi) {
        // We model the CPU as of 2025-10-01 as "[almost] all possible threads"
        // [0, UINT32_MAX) and if we pass that here, we will create 4 billion
        // VisRecords.
        CAMSPORK_REQUIRE_CMP(tid_hi, <, UINT32_MAX,
            "Likely you meant to pass convergent_access_flag");
        ...
        CAMSPORK_REQUIRE_CMP((granularity - 1) & granularity, ==, 0,
            "Must be positive power of 2");
        CAMSPORK_REQUIRE(access.is_ooo || granularity == 1,
            "out-of-order non-convergent abstract machine optimization not "
            "applicable when !is_ooo");

        for (uint32_t tid = tid_lo & ~(granularity - 1); tid < tid_hi;
             tid += granularity) {
          const VisRecordID new_vis_record_id = memoize_new_vis_record<K>(
              SimpleThreadInit{tid, tid + granularity}, access,
              vis_record_refcnt);
          logger.history_new_vis_record(*this, new_vis_record_id);
    ...
    logger.excut_log_assignment_records(
        *this, input, new_vis_record_list, IsMutate);
    ...
    auto check = [&](node_id id, size_t linear_index) {
      ...
      const auto mut_vis_flag_needed =
          (access.atomic_qual_bits != 0 ? vis_flag_atomic_only
              : access.is_write_only    ? vis_flag_temporal
                                        : vis_flag_full);

      // Check against previous mutate visibility records
      LoggedMissingTlSig fail_tl_sig{};
      nodepool::id<DefaultAssignmentRecordVisNode> mutate_id =
          assignment_record.mutate_vis_records_head_id;
      while (mutate_id) {
        DefaultAssignmentRecordVisNode &node = get(mutate_id);
        const VisRecord &mutate_record = remove_forwarding(&node.vis_record_id);
        logger.history_vis_record_checked(
            node.vis_record_id, IsMutate);  // Logs memoized (base state) ID.
        const bool visible =
            visible_to(mutate_record, cuboid, access.extended_qual_bits,
                mut_vis_flag_needed, is_convergent, &fail_tl_sig);
        if (!visible) {
          logger.history_vis_record_error(node.vis_record_id, fail_tl_sig);
          throw SyncvCheckFail{
              IsMutate ? "WAW HAZARD" : "RAW HAZARD", linear_index};
        }
        mutate_id = node.camspork_next_id;
      }

      // If the access is a mutate, also check against the list of previous read
      // visibility records.
      if constexpr (IsMutate) {
        nodepool::id<DefaultAssignmentRecordVisNode> read_id =
            assignment_record.read_vis_records_head_id;
        while (read_id) {
          ...
          logger.history_vis_record_checked(
              node.vis_record_id, IsMutate);  // Logs memoized (base state) ID.
          const bool visible =
              visible_to(read_record, cuboid, access.extended_qual_bits,
                  vis_flag_temporal, is_convergent, &fail_tl_sig);
          if (!visible) {
            logger.history_vis_record_error(node.vis_record_id, fail_tl_sig);
    ...
    auto extend_vis_records =
        [&](nodepool::id<AssignmentRecordVisNode<K>> *p_list_head) {
          for (const VisRecordID vis_record_id : new_vis_record_list) {
            ...
            DefaultAssignmentRecordVisNode &node =
                alloc_default_node(&new_node_id);
    ...
    auto copy_on_write_update = [&](node_id old_id,
                                    AssignmentRecordCensusEntry &entry) {
      // If the old assignment record has ID 0 (doesn't exist ... was presumed
      // empty, no reads, no mutates) or its refcnt exceeds the use count, then
      // we cannot modify the assignment record in-place. Save the ID of the
      // replacement assignment record (new_id), which is the same as the old_id
      // if in-place.
      ...
      if (old_id) {
        ...
        if (can_modify) {
          ...
          // Make a copy of the old AssignmentRecord, and transfer entry.count
          // owning references from the old to the new one.
      ...
      // Add new visibility record (either as new mutate visibility record, or
      // appended read visibility record).
      if constexpr (IsMutate) {
        ...
        assignment_record_remove_vis_records(
            &assignment_record.read_vis_records_head_id);
        if (access.atomic_qual_bits != 0) {
          extend_vis_records(&assignment_record.mutate_vis_records_head_id);
          lazy_remove_duplicates(
              &assignment_record);  // << IMPORTANT for performance
        } else {
          assignment_record_remove_vis_records(
              &assignment_record.mutate_vis_records_head_id);
          extend_vis_records(&assignment_record.mutate_vis_records_head_id);
          assignment_record.lazy_last_augment_counter_bits =
              get_augment_counter_bits();
        }
      } else {
        // Add the new visibility records to the list of read visibility
        // records.
    ...
    // Write out new assignment record IDs. Reference counting is already taken
    // care of.
    if constexpr (!UpdateRecords) {
      ...
      const nodepool::id<AssignmentRecord> new_id =
          census[0].second.new_node_id;
      input->node_id =
          new_id.id_bits;  // Where decltype(input) is assignment_record_id*
  ...
  void on_r(Input input, const ThreadCuboid &cuboid, SyncvAccessInfo access,
      Logger &&logger) {
    if (no_checking_counter != 0) {
    } else if (access.is_convergent) {
      checked_on_access_impl<false, true, true>(
          access.is_convergent, input, cuboid, access, logger);
    } else {
      checked_on_access_impl<false, false, true>(
          access.is_convergent, input, cuboid, access, logger);
    }
  }
  ...
  // Expect Input = assignment_record_id* or AssignmentRecordWindow.
  template <typename Input, typename Logger>
  void on_rw(Input input, const ThreadCuboid &cuboid, SyncvAccessInfo access,
      Logger &&logger) {
    if (no_checking_counter != 0) {
    } else if (access.is_convergent) {
      checked_on_access_impl<true, true, true>(
          access.is_convergent, input, cuboid, access, logger);
    } else {
      checked_on_access_impl<true, false, true>(
          access.is_convergent, input, cuboid, access, logger);
    }
  }
  ...
  // Expect Input = assignment_record_id* or AssignmentRecordWindow.
  template <typename Input, typename Logger>
  void on_check_free(Input input, const ThreadCuboid &cuboid,
      SyncvAccessInfo access, Logger &&logger) {
    if (no_checking_counter != 0) {
    } else {
      // SharedVisRecord doesn't matter when UpdateRecords=false.
      checked_on_access_impl<true, true, false>(
          access.is_convergent, input, cuboid, access, logger);
    }
  }

  void clear_visibility(
      size_t N, assignment_record_id *p_assignment_record_ids) {
    for (size_t i = 0; i < N; ++i) {
  ...
  // Removing forwarding causes two equivalent visibility records to have
  // identical IDs (both referring to the shared entry in the memoization
  // table).
  template <VisRecordKind K>
  void remove_duplicates(
    ...
    // Remove forwarding (unique ID iff unique record), and clear
    // tmp_is_duplicate to 0. Importantly, we are clearing this flag for
    // base-state VisRecord, not forwarded ones.
    ...
    while (node_id next_id = *p_read_id) {
      ...
      CAMSPORK_REQUIRE(!vis_record_node.is_forwarded(),
          "should have resolved forwarding above");

      if (vis_record_node.tmp_is_duplicate) {
        // Duplicate, remove next node from list (decrements refcount for
        // duplicated vis record). This causes (next_id = *p_read_id) to change,
        // so we don't have to update p_read_id. i.e. since we removed the next
        // node, we're ready to process a new next node next iteration.
        node_id victim_id = remove_next_node(p_read_id);
        CAMSPORK_REQUIRE_CMP(
            victim_id, ==, next_id, "didn't remove expected node");
        CAMSPORK_REQUIRE(!next_node.camspork_next_id,
            "next_node should have been removed above");
...
// If we leave things as-is, read vis records may build up indefinitely for
// variables that are written once and read many times. We fix this by removing
// duplicates; however, this is really expensive, so we only do it once after
// each fence or await event (synchronization is when memoization kicks in to
// potentially allow us to recognize duplicates due to duplicated IDs).
#pragma GCC diagnostic push
#pragma GCC diagnostic ignored "-Wconversion"
    const auto old_bits = p_assignment_record->lazy_last_augment_counter_bits;
    p_assignment_record->lazy_last_augment_counter_bits =
        get_augment_counter_bits();
#pragma GCC diagnostic pop
    ...
    if (old_bits != p_assignment_record->lazy_last_augment_counter_bits) {
      ...
      // However, this is unlikely, and is only a performance issue if so (we
      // fail to remove duplicates).
  ...
  void debug_get_read_vis_record_ids(
      const AssignmentRecord &record, std::vector<uint32_t> *out) const {
    out->clear();
    nodepool::id<DefaultAssignmentRecordVisNode> id =
        record.read_vis_records_head_id;
    while (id) {
  ...
  template <VisRecordKind K>
  void debug_get_vis_record_data(nodepool::id<VisRecordListNode<K>> node_id,
      VisRecordDebugData *out) const {
    ...
    for (nodepool::id<TlSigIntervalListNode> node_id = record.visibility_set;
         node_id;) {
      const TlSigIntervalListNode &node = get(node_id);
    ...
    for (nodepool::id<PendingAwaitNode> node_id = record.pending_awaits;
         node_id;) {
  ...
  struct RefcntDebug {
    ...
    void check_refcnts(const SyncvTable &self) {
      for (nodepool::id<ListNode> id : self.debug_get_pool<ListNode>()) {
        ...
        if (is_free) {
          CAMSPORK_REQUIRE_CMP(
              true_refcnt, ==, 0, "Reference exists to free node");
        } else {
  ...
  // This only works if all of the user's arrays of assignment_record_id have
  // been passed. NB this is a const member function to help guard against
  // accidental subtle changes in the course of checking which could cause
  // heisenbugs.
  void debug_validate_state(
      size_t input_count, const SyncvDebugValidateInput *p_inputs) const {
    fprintf(stderr, "SyncvTable::debug_validate_state\n");

    CAMSPORK_REQUIRE_CMP(modified_vis_records.size(), ==, 0,
        "internal error, missing memoize_modified()");

    std::tuple<RefcntDebug<AssignmentRecord>,
        RefcntDebug<TlSigIntervalListNode>, RefcntDebug<PendingAwaitNode>,
        RefcntDebug<DefaultVisRecordListNode>,
        RefcntDebug<DefaultAssignmentRecordVisNode>>
        debug_refcnts(*this, *this, *this, *this, *this);
    ...
    {
      std::vector<refcnt_t> &refcnts =
          std::get<RefcntDebug<typename decltype(id)::value_type>>(
              debug_refcnts)
              .refcnts;
      if (id) {
        CAMSPORK_REQUIRE_CMP(
            id.node_index(), <, refcnts.size(), "out-of-bounds node ID");
    ...
    auto process_assignment_record = [&](nodepool::id<AssignmentRecord> id,
                                         auto recurse) {
      ...
      const nodepool::id<DefaultAssignmentRecordVisNode> mutate_id =
          record.mutate_vis_records_head_id;
      process_assignment_record_list(mutate_id);
      const nodepool::id<DefaultAssignmentRecordVisNode> read_id =
          record.read_vis_records_head_id;
    ...
    // Further count ownership references from AssignmentRecord to
    // VisRecordListNode, AssignmentRecordVisNode
    ...
    for (uint32_t barrier_index = 0; barrier_index < max_live_barriers;
         ++barrier_index) {
      const BarrierState &state = barrier_states[barrier_index];
      for (const auto &pair : state.arrive_states) {
        const nodepool::id<DefaultAssignmentRecordVisNode> head_id =
            pair.second.vis_records_head_id;
        process_assignment_record_list(head_id);
      }
    }

    // This handles references between PendingAwaitNode
    auto on_PendingAwaitNode = [&](nodepool::id<PendingAwaitNode> id,
                                   auto recurse) {
      if (!id) {
        return;
      }
    ...
    // Count ownership references from live VisRecordListNode objects to other
    // objects:
    ...
    //   * stored hash is correct (could be wrong if we forgot to memoize on
    //   change).
    auto process_vis_record_impl = [&](auto id, const auto &free_vis_ids) {
      ...
      if (node.is_forwarded()) {
        CAMSPORK_REQUIRE(node.camspork_next_id,
            "in forwarding state, but forwarded-to node is null");
        record_owning(node.camspork_next_id);
        CAMSPORK_REQUIRE(!node.base_data.visibility_set,
            "state should have been cleared upon forwarding");
        CAMSPORK_REQUIRE(!node.base_data.pending_awaits,
            "state should have been cleared upon forwarding");
      } else {
        const auto stored_hash = read_hash_helper(id);
        CAMSPORK_REQUIRE_CMP(
            stored_hash, ==, hash_vis_record(node.base_data), "dirty hash");

        uint32_t max_non_atomic_tid = 0;
        for (nodepool::id<TlSigIntervalListNode> node_id =
                 node.base_data.visibility_set;
             node_id;) {
          ...
          for (uint32_t vis_flag_index = 0; vis_flag_index < num_vis_flags;
               ++vis_flag_index) {
            const auto q_bits = q.array[vis_flag_index];
            if (vis_flag_index != vis_flag_index_atomic_only && q_bits != 0) {
              max_non_atomic_tid =
                  std::max(max_non_atomic_tid, this_node.data.tid_hi - 1u);
            }
          }
          ...
          if (next_id) {
            ...
            CAMSPORK_REQUIRE(valid_adjacent(this_node.data, next_node.data),
                "visibility set intervals not sorted properly");
            node_id = next_id;
          } else {
            break;
          }
        }
        CAMSPORK_REQUIRE_CMP(max_non_atomic_tid_from_hash(stored_hash), ==,
            max_non_atomic_tid,
        ...
        // The other half of this checking is done in
        // check_BarrierArriveState_VisRecords.
        MultiSet<decltype(PendingAwaitNode::await_id)> await_id_multiset;
        nodepool::id<PendingAwaitNode> await_node_id =
            node.base_data.pending_awaits;
        while (await_node_id) {
        ...
        for (auto await_id : await_id_multiset) {
          const BarrierArriveState *p_state =
              get_const_barrier_arrive_state(await_id);
          if (!p_state) {
            fprintf(stderr, "%u\n", id.id_bits);
          }
          ...
          nodepool::id<AssignmentRecordVisNode<K>> record_node_id =
              state.vis_records_head_id;
          size_t count = 0;
          ...
          CAMSPORK_REQUIRE_CMP(count, ==, await_id_multiset.count(await_id),
              "Expect 1:1 VisRecord->PendingAwait and PendingAwait->VisRecord "
              "references");
        }
      }
    };
    ...
    auto process_all_vis_records = [&](auto id_for_typing) {
      ...
      RefcntDebug<ListNode> &debug_info =
          std::get<RefcntDebug<ListNode>>(debug_refcnts);
    ...
    // For node types without refcnt, the refcnt should just be 0 or 1 (unique
    // ownership).
    check_all_refcnts();

    // Memoization Validation
    // A VisRecord should be in the memoization table iff it's alive and in the
    // base state.
    ...
    auto validate_chunk = [this, &last_hash_bits](const auto &chunk) {
      ...
      CAMSPORK_REQUIRE_CMP(read_hash_helper(chunk.nodes.back()), ==,
          chunk.max_hash, "Wrong chunk.max_hash");
      for (nodepool::id<VisRecordListNode<K>> id : chunk.nodes) {
        ...
        CAMSPORK_REQUIRE_CMP(node.refcnt, !=, 0,
            "all memoized VisRecord should have nonzero refcnt");
        CAMSPORK_REQUIRE(!node.is_forwarded(),
            "forwarding state VisRecord should not be memoized");
        CAMSPORK_REQUIRE(!node.camspork_next_id, "unexpected linked list next");
        CAMSPORK_REQUIRE_CMP(
            node.memoize_hash_bits, ==, expect_hash_bits, "wrong hash");
        CAMSPORK_REQUIRE_CMP(
            last_hash_bits, <=, expect_hash_bits, "Not sorted");
        last_hash_bits = expect_hash_bits;
      }
    };

    last_hash_bits = 0;
    for (const VisRecordChunk<VisRecordKind::Default> &chunk :
        vis_record_table) {
      validate_chunk(chunk);
    }

    // (VisRecord in memoization table <- alive and in base state)
    // Each VisRecord should be able to find itself in the table; if we fail, it
    // could be because we forgot to memoize it, or something is wrong with the
    // hash search or equality function.
    auto memoize_self_check = [&](auto id_for_typing) {
      using ListNode = typename decltype(id_for_typing)::value_type;
      RefcntDebug<ListNode> &debug =
          std::get<RefcntDebug<ListNode>>(debug_refcnts);
      for (nodepool::id<ListNode> id : debug_get_pool<ListNode>()) {
        ...
        try {
          CAMSPORK_REQUIRE_CMP(
              id, ==, find_memoized(id), "memoization lookup is buggy");
        } catch (...) {
          for (size_t chunk_index = 0; chunk_index < vis_record_table.size();
               ++chunk_index) {
    ...
    // A base state VisRecord is pointed to by BarrierArriveState iff it
    // contains a corresponding pending await. BarrierArriveState may also point
    // to forwarding state VisRecord. The other half of this checking is in
    // process_vis_record_impl.
    auto check_BarrierArriveState_VisRecords =
        [&](auto record_node_id, pending_await_t expected_await_id) {
          while (record_node_id) {
            constexpr VisRecordKind K =
                decltype(record_node_id)::value_type::vis_record_kind;
  ...
  void debug_print(
      FILE *f, const VisRecord &record, unsigned long long hash) const {
    ...
    while (interval_id) {
      ...
      const TlSigInterval data = node.data;
      fprintf(f, "[%u, %u, %u, %u, %u, %u]\n", data.tid_lo, data.tid_hi,
          data.qual_bits_by_vis.array[0], data.qual_bits_by_vis.array[1],
          data.qual_bits_by_vis.array[2], data.qual_bits_by_vis.array[3]);
    }
...
struct SyncvTrivialLogger;  // Defined before SyncvTable; matches
                            // SyncvRealLogger interface.
...
struct SyncvRealLogger {
  ...
  void excut_log_assignment_records(const SyncvTable &env,
      assignment_record_id *p_id, const VisRecordList &new_vis_record_list,
      bool is_mutate) {
    if (p_excut_actions) {
      nodepool::id<AssignmentRecord> asn_id{p_id->node_id};
      _excut_log_assignment_record_impl(
          env, asn_id, new_vis_record_list, idx_for_single, is_mutate);
    }
  }

  template <typename VisRecordList>
  void excut_log_assignment_records(const SyncvTable &env,
      AssignmentRecordWindow window, const VisRecordList &new_vis_record_list,
      bool is_mutate) {
    if (p_excut_actions) {
      std::vector<extent_t> idx(
          window.end_outer_extent - window.begin_outer_extent);
      _excut_recurse_log_window(
          env, window, idx, 0, 0, new_vis_record_list, is_mutate);
    }
  }
  ...
  void excut_update_assignment_record_ids(const CensusMap &census) {
    for (ExcutSyncEnvAccess *p : actions_to_update) {
      ...
      CAMSPORK_REQUIRE(iter != census.end(),
          "ExcutSyncEnvAccess::id_before not found in CensusMap");
      const AssignmentRecordCensusEntry &entry = iter->second;
      p->id_after = entry.new_node_id.id_bits;
    }
  }

  void excut_update_assignment_record_ids(
      nodepool::id<AssignmentRecord> new_id) {
    for (ExcutSyncEnvAccess *p : actions_to_update) {
...
private:
  template <typename VisRecordList>
  void _excut_recurse_log_window(const SyncvTable &env,
      const AssignmentRecordWindow &window, std::vector<extent_t> &idx,
      size_t dim_idx, size_t partial_linear_offset,
      const VisRecordList &new_vis_record_list, bool is_mutate) {
    if (dim_idx >= idx.size()) {
      CAMSPORK_REQUIRE_CMP(idx.size(), ==, dim_idx, "overshot");
      nodepool::id<AssignmentRecord> asn_id{
          window.base[partial_linear_offset].node_id};
      _excut_log_assignment_record_impl(
          env, asn_id, new_vis_record_list, idx, is_mutate);
    } else {
      ...
      for (extent_t i = offset_c; i < end_c; ++i) {
        ...
        _excut_recurse_log_window(env, window, idx, dim_idx + 1,
            new_linear_offset, new_vis_record_list, is_mutate);
      }
    }
  }

  template <typename VisRecordList>
  void _excut_log_assignment_record_impl(const SyncvTable &env,
      nodepool::id<AssignmentRecord> asn_id,
      const VisRecordList &new_vis_record_list, std::vector<extent_t> idx,
      bool is_mutate) {
    constexpr VisRecordKind K =
        VisRecordList::value_type::value_type::vis_record_kind;
    ...
    // and remember to update this with the changed ID later.
    {
      ...
      p_info->mutate_tag =
          is_mutate ? ExcutMutateTag::Mutate : ExcutMutateTag::Read;
      actions_to_update.push_back(p_info.get());
      p_excut_actions->push_back(std::move(p_info));
    }

    // Log existing VisRecords, tagged as WAR, WAW, or RAW, depending on the
    // relation between the prior VisRecords and the current SyncEnvAccess
    // action.
    if (asn_id) {
      const AssignmentRecord &asn_record = env.get(asn_id);
      if (is_mutate) {
        nodepool::id<DefaultAssignmentRecordVisNode> read_id =
            asn_record.read_vis_records_head_id;
        while (read_id) {
          const DefaultAssignmentRecordVisNode &asn_node = env.get(read_id);
          read_id = asn_node.camspork_next_id;
          _excut_log_vis_record(
              env, asn_node.vis_record_id, ExcutMutateTag::WAR);
        }
      }
      nodepool::id<DefaultAssignmentRecordVisNode> mutate_id =
          asn_record.mutate_vis_records_head_id;
      while (mutate_id) {
        const DefaultAssignmentRecordVisNode &asn_node = env.get(mutate_id);
        mutate_id = asn_node.camspork_next_id;
        _excut_log_vis_record(env, asn_node.vis_record_id,
            is_mutate ? ExcutMutateTag::WAW : ExcutMutateTag::RAW);
      }
    }

    // Log new VisRecord
    for (nodepool::id<VisRecordListNode<K>> new_vis_id : new_vis_record_list) {
      _excut_log_vis_record(env, new_vis_id,
          is_mutate ? ExcutMutateTag::Mutate : ExcutMutateTag::Read);
    }
  }

  template <VisRecordKind K>
  void _excut_log_vis_record(const SyncvTable &env,
      nodepool::id<VisRecordListNode<K>> id, ExcutMutateTag mutate_tag) {
  ...
  void _excut_log_vis_record_data(const SyncvTable &env,
      const VisRecord &vis_record, ExcutMutateTag mutate_tag) {
    ...
    while (await_node_id) {
      ...
      env.set_barrier_index(
          &tmp_barrier_id, pending_await_barrier_index(node.await_id));
...
public:
  ...
  void history_set_sync_stmt_info(const SyncvArrive &arrive,
      const BarrierState &state, uint32_t new_arrive_count) {
    if (p_history_log) {
      LoggedSyncStmtValues values{};
      ...
      p_history_log->set_syncv_sync_stmt_info(
          arrive.home_barrier, std::move(values));
    }
  }

  void history_set_sync_stmt_info(const SyncvAwait &await,
      const BarrierState &state, uint32_t new_await_count,
      uint32_t max_arrive_count) {
    if (p_history_log) {
      LoggedSyncStmtValues values{};
  ...
  template <VisRecordKind K>
  void history_new_vis_record(
      SyncvTable &env, nodepool::id<VisRecordListNode<K>> node_id) {
    if (p_history_log) {
      history_log_vis_record_id history_id(node_id);
      p_history_log->log_syncv_new_vis_record(
          history_id, _get_history_vis_record_data(env, node_id));
    }
  }

  template <VisRecordKind K>
  void history_vis_record_change(SyncvTable &env,
      nodepool::id<VisRecordListNode<K>> old_id,
      nodepool::id<VisRecordListNode<K>> new_id, bool debug_printf) {
    if (p_history_log) {
      p_history_log->log_syncv_vis_record_change(
          history_log_vis_record_id(old_id), history_log_vis_record_id(new_id),
          _get_history_vis_record_data(env, new_id), debug_printf);
    }
  }

  template <VisRecordKind K>
  void history_vis_record_checked(
      nodepool::id<VisRecordListNode<K>> id, bool is_mutate) {
    if (p_history_log) {
      p_history_log->log_syncv_vis_record_checked(
          history_log_vis_record_id(id), is_mutate);
    }
  }

  template <VisRecordKind K>
  void history_vis_record_error(
      nodepool::id<VisRecordListNode<K>> id, LoggedMissingTlSig fail_tl_sig) {
    if (p_history_log) {
      p_history_log->log_syncv_vis_record_error(
          history_log_vis_record_id(id), fail_tl_sig);
    }
  }

private:
  template <VisRecordKind K>
  LoggedVisRecordData _get_history_vis_record_data(
      SyncvTable &env, nodepool::id<VisRecordListNode<K>> node_id) {
    ...
    for (size_t i = 0; i < pending_awaits.size(); ++i) {
      ...
      data.pending_await_list.push_back(LoggedPendingAwait{
          p_history_log->get_barrier_name(id), arrive_count});
    }
    return data;
  }
};
...
#define INTERFACE_PROLOGUE(table)                                              \
  try {                                                                        \
    CAMSPORK_REQUIRE(                                                          \
        !table->failed, "Cannot continue using env after failure detected");

#define INTERFACE_EPILOGUE(table)                                              \
  CAMSPORK_REQUIRE_CMP(table->modified_vis_records.size(), ==, 0,              \
      "internal error, missing memoize_modified()");                           \
  }                                                                            \
  catch (...) {                                                                \
    table->failed = true;                                                      \
...
void SyncvTableDeleter::operator()(SyncvTable *victim) const { delete victim; }

void on_r(SyncvTable *table, assignment_record_id *input,
...
void on_r(SyncvTable *table, AssignmentRecordWindow input,
    const ThreadCuboid &cuboid, SyncvAccessInfo access, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
  table->on_r(input, cuboid, access, SyncvTrivialLogger{});
  INTERFACE_EPILOGUE(table)
...
void on_rw(SyncvTable *table, assignment_record_id *input,
    const ThreadCuboid &cuboid, SyncvAccessInfo access, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
...
void on_rw(SyncvTable *table, AssignmentRecordWindow input,
    const ThreadCuboid &cuboid, SyncvAccessInfo access, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
  table->on_rw(input, cuboid, access, SyncvTrivialLogger{});
  INTERFACE_EPILOGUE(table)
...
void on_check_free(SyncvTable *table, AssignmentRecordWindow input,
    const ThreadCuboid &cuboid, SyncvAccessInfo access, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
...
void clear_visibility(
    SyncvTable *table, size_t N, assignment_record_id *array) {
  INTERFACE_PROLOGUE(table)
...
void on_fence(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvFence &fence, const SyncvLogRequest &log) {
  INTERFACE_PROLOGUE(table)
...
void on_fence(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvFence &fence, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
...
void on_arrive(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvArrive &arrive, const SyncvLogRequest &log) {
  INTERFACE_PROLOGUE(table)
...
void on_arrive(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvArrive &arrive, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
...
void on_await(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvAwait &await, const SyncvLogRequest &log) {
  INTERFACE_PROLOGUE(table)
...
void on_await(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvAwait &await, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
...
void on_join_threads(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvJoinThreads &join_threads, const SyncvLogRequest &log) {
  INTERFACE_PROLOGUE(table)
  table->on_join_threads(cuboid, join_threads, SyncvRealLogger(log));
  INTERFACE_EPILOGUE(table)
}

void on_join_threads(SyncvTable *table, const ThreadCuboid &cuboid,
    const SyncvJoinThreads &join_threads, decltype(nullptr)) {
  INTERFACE_PROLOGUE(table)
  table->on_join_threads(cuboid, join_threads, SyncvTrivialLogger{});
  INTERFACE_EPILOGUE(table)
}

void begin_no_checking(SyncvTable *table) { table->no_checking_counter++; }

void end_no_checking(SyncvTable *table) {
  CAMSPORK_REQUIRE(
      table->no_checking_counter, "end_no_checking without begin_no_checking");
...
void debug_get_vis_record_data(
  ...
  table->debug_get_vis_record_data(
      nodepool::id<VisRecordListNode<VisRecordKind::Default>>{id}, out);
}
