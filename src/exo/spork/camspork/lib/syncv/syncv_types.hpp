...
struct assignment_record_id {
  ...
  explicit operator bool() const { return node_id != 0; }
};
...
struct barrier_id {
  ...
  bool operator<(barrier_id other) const { return data < other.data; }
};
...
struct SyncvCheckFail : std::runtime_error {
  ...
  size_t linear_index_in_input() const { return _linear_index_in_input; }
};
...
struct ThreadCuboid {
  ...
  static ThreadCuboid full(Iterator begin, Iterator end) {
    ...
    CAMSPORK_REQUIRE_CMP(dim, <=, max_dim,
        "implementation limit: ThreadCuboid::max_dim exceeded");
  ...
  void to_intervals(Callback &&callback) const {
    ...
    cuboid_to_intervals<uint32_t>(domain(), domain() + dim(), offset(),
        offset() + dim(), box(), box() + dim(),
  ...
  void reshape(uint32_t new_dim, const uint32_t *new_domain) {
    CAMSPORK_REQUIRE_CMP(new_dim, <=, max_dim,
        "Implementation limit exceeded: maximum DomainReshape dimensions");
    CAMSPORK_REQUIRE_CMP(
        new_dim, >=, dim(), "Invalid reshape that reduces dimensionality");

    uint32_t new_num_threads = 1;
    for (uint32_t i = 0; i < new_dim; ++i) {
      CAMSPORK_REQUIRE_CMP(
          new_domain[i], >=, 2, "Domain coordinate must be at least 2");
      new_num_threads *= new_domain[i];
    }
    CAMSPORK_REQUIRE_CMP(new_num_threads, ==, domain_num_threads(),
        "wrong thread count for DomainReshape");
    ...
    for (uint32_t old_i = dim(); old_i > 0;) {
      ...
      while (domain_prod < old_domain_c) {
        CAMSPORK_REQUIRE_CMP(
            new_i, >, 0, "Shouldn't happen given earlier thread count check");
        --new_i;
        ...
        if (box_quot <= new_domain_c) {
          ...
          CAMSPORK_REQUIRE_CMP(box_quot % new_domain_c, ==, 0,
              "thread box misaligned for DomainReshape");
      ...
      CAMSPORK_REQUIRE_CMP(domain_prod, ==, old_domain_c,
          "DomainReshape can only split dimensions");
    }
...
struct SimpleThreadInit {
  ...
  TlSigBucketKey minimal_superset_interval() const { return {tid_lo, tid_hi}; }
};

template <typename Stream>
Stream &&operator<<(Stream &&s, const ThreadCuboid &cuboid) {
  const uint32_t dim = cuboid.dim();
  CAMSPORK_REQUIRE_CMP(dim, <=, cuboid.max_dim,
      "tried to print ThreadCuboid with too many dimensions");
