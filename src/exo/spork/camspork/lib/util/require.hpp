...
[[noreturn]] void require_cmp_fail(const char *file, int line, LHS lhs, RHS rhs,
    const char *lhs_name, const char *op, const char *rhs_name,
    const char *msg) {
  try {
    std::stringstream s;
...
#define CAMSPORK_REQUIRE(expr, msg)                                            \
  do {                                                                         \
    if (!(expr)) {                                                             \
...
#define CAMSPORK_REQUIRE_CMP(lhs, op, rhs, msg)                                \
  do {                                                                         \
    if (!((lhs)op(rhs))) {                                                     \
      ::camspork::require_cmp_fail(                                            \
          __FILE__, __LINE__, lhs, rhs, #lhs, #op, #rhs, msg);                 \
    }                                                                          \
  } while (0)

#define CAMSPORK_C_BOUNDSCHECK(i, bounds)                                      \
  CAMSPORK_REQUIRE_CMP(size_t(i), <, size_t(bounds),                           \
      "this may be a bug in the C library, or a corrupt input file")
