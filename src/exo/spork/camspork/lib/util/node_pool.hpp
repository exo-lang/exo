...
// The bottom CAMSPORK_NODE_POOL_DEBUG_BITS-many bits are reserved to detect
// use-after-free. Each time a node is freed, we update the expected debug bits
// for the node at that index so we can detect usage of the old stale ID.
...
struct id {
  ...
  explicit operator bool() const { return id_bits != 0; }
  ...
  bool operator==(id other) const { return id_bits == other.id_bits; }
...
constexpr uint32_t pack_id_bits(uint32_t node_index, uint8_t debug_bits) {
  const uint32_t index_bits_32 = (node_index + 1)
                                 << CAMSPORK_NODE_POOL_DEBUG_BITS;
  if constexpr (CAMSPORK_NODE_POOL_DEBUG_BITS > 0) {
    const uint32_t debug_bits_32 =
        debug_bits & ((1u << CAMSPORK_NODE_POOL_DEBUG_BITS) - 1u);
...
constexpr uint32_t pack_id_bits(
    uint32_t node_index, uint32_t debug_bits) = delete;
...
public:
  ...
  Pool &operator=(const Pool &other) { return (*this = Pool(other)); };
    ...
    if constexpr (CAMSPORK_NODE_POOL_DEBUG_BITS) {
      if (!no_debug) {
        ...
        CAMSPORK_REQUIRE_CMP(
            _id.id_bits, ==, expected_id_bits, "likely use-after-free of node");
      }
    }
  ...
  ListNode &alloc_default_node(
      uintptr_t *p_memory_budget, id<ListNode> *out_id) {
    if (!free_list_head) {
      ...
      // ID overflow check
      CAMSPORK_REQUIRE_CMP(chunks.size() * chunk_size, <, max_node_index(),
          "node ID int overflow");
      ...
      *p_memory_budget -= sizeof new_chunk;
      ...
      for (uint32_t i = 0; i < chunk_size - 1; ++i) {
        new_chunk.storage[i].camspork_next_id.id_bits =
            pack_id_bits(id_offset + i, uint8_t(0));
        if constexpr (CAMSPORK_NODE_POOL_DEBUG_BITS > 0) {
      ...
      free_list_head.id_bits =
          pack_id_bits(old_chunk_count * chunk_size, uint8_t(0));

      CAMSPORK_REQUIRE_CMP(&new_chunk.storage[0], ==,
          &get(free_list_head, true), "node free list broken");
    }
  ...
  void extend_free_list(id<ListNode> head_id) noexcept {
    ...
    while (1) {
      ...
      if constexpr (CAMSPORK_NODE_POOL_DEBUG_BITS) {
        ...
        chunks[node_index / chunk_size]
            ->expected_debug_bits[node_index % chunk_size]--;
      }
  ...
  void insert_next_node(
      id<ListNode> *p_insert_after, id<ListNode> insert_me) noexcept {
  ...
  id<ListNode> id_from_node_index(uint32_t node_index) const {
    ...
    if constexpr (CAMSPORK_NODE_POOL_DEBUG_BITS) {
      debug_bits = chunks[node_index / chunk_size]
                       ->expected_debug_bits[node_index % chunk_size];
    }
...
// Debug feature: you will get IDs to nodes on the free list that appear valid
// to the use-after-free checker. The caller must have a way to distinguish such
// nodes.
template <typename ListNode>
