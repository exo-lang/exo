...
class SMTSolver:
    ...
    def _newvar(self,sym, typ=T.index, ternary=False):
        ...
        if self.Z3_MODE:
            ...
            if ternary:
                self.env[sym] = TernVal(smt_sym,
                                        SMT.Symbol(nm+"_def", SMT.BOOL))
            else:
                self.env[sym] = smt_sym
            return self.env[sym]
            ...
            if len(tmp_bin) > 0:
                ...
                if self.Z3_MODE:
                    ...
                    if self.negative_pos[id(e)] == '+':
                        smt_e   = Z3.ForAll(all_syms,
                                            Z3.Implies(all_eq,smt_e))
                    else:
                        smt_e   = Z3.Exists(all_syms,Z3.And(all_eq,smt_e))
                else:
                    all_eq      = SMT.And(*[ eq  for sym,eq in tmp_bin])
                    if self.negative_pos[id(e)] == '+':
                        smt_e   = SMT.ForAll(all_syms,
                                             SMT.Implies(all_eq,smt_e))
                    else:
    ...
    def _lower_body(self, e):
        if isinstance(e, A.Const):
            ...
            if self.Z3_MODE:
                val     = (Z3.BoolVal(False) if e.type == T.bool else
                           Z3.IntVal(0))
            ...
            if is_ternary(cond) or is_ternary(tcase) or is_ternary(fcase):
                ...
                if self.Z3_MODE:
                    return TernVal(Z3.If(c.v, t.v, f.v),
                                   Z3.And(c.d, Z3.If(c.v,t.d,f.d)))
                else:
            ...
            if self.Z3_MODE:
                ...
                if is_ternary(a):
                    ...
                    is_def  = Z3.Or( Z3.ForAll([nm], a.d),
                                     Z3.Exists([nm], Z3.And(short_a, a.d)) )
            ...
            if e.op == "+":
                ...
                if e.lhs.type == T.bool and e.rhs.type == T.bool:
                    val = ((lhs == rhs) if self.Z3_MODE else
                           SMT.Iff(lhs, rhs))
