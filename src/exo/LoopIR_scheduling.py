...
def DoBindExpr(new_name, expr_cursors, cse=False):
    ...
    alloc_s = LoopIR.Alloc(new_name, expr.type, None, None, expr.srcinfo)
    assign_s = LoopIR.Assign(new_name, expr.type, None, [], expr, None, expr.srcinfo)
...
def DoStageMem(block_cursor, buf_name, w_exprs, new_name, use_accum_zero=False):
    ...
    # Get condition to ensure load/store stages don't access out of bounds
    def get_safety_cond(ir, fwd, ctxt_stmt_c, access, buf_typ):
        def check_cond(cond):
    ...
    if isW:
        ...
        ir, fwd = get_safety_cond(ir, fwd, store_stmt_c, store_stmt_c._node, buf_typ)
