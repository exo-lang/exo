...
If we number each read starting from the first one S-after the S-last write (and ignore all S-prior reads), then the Nth read visibility record's $V_S$ stores the set of sigthreads that ``observed'' the Nth read, in the sense that performing a write will not cause a WAR hazard with said read.
...
\begin{enumerate}
  ...
  \item $V_S$ set to the write's accessor set if the instruction is synchronous, empty otherwise;
