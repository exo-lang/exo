...
\filbreak
\myKey{Actor Kind:} In Exo currently, we assume all code is executed on the CPU, or with accelerator functions that operate \textit{synchronously} with the CPU.
...
For now I'm proposing that we use the term ``actor kind'' for this, since in a dynamic trace of a program, we're labeling each \textit{action} with the kind of \textit{actor} that executed it.

\filbreak
The default CPU and CUDA actor kinds are sequential; all others are asynchronous actor kinds.

Note, this was formerly something I called ``execution model'' (xmodel).
...
\begin{align*}
\texttt{cpu: } & \text{Default CPU}\\
\graytt{cuda\_all: } & \text{All actions on a CUDA device}\\
\texttt{cuda\_sync: } & \text{Default CUDA (synchronous CUDA instructions)}\\
\texttt{non\_bulk\_cp\_async: } & \text{Ampere \lighttt{cp.async}, (async memcpy in generic proxy)}\\
\graytt{cuda\_generic: } & \text{Generic proxy; \lighttt{cuda\_sync} $\cup$ \lighttt{non\_bulk\_cp\_async}}\\
\texttt{tma\_to\_shared\_async: } & \text{Hopper TMA \lighttt{cp.bulk.async} to shared memory}\\
\texttt{tma\_to\_global\_async: } & \text{Hopper TMA \lighttt{cp\{.reduce\}.bulk.async} to global memory}\\
\texttt{wgmma\_async: } & \text{wgmma instructions}\\
\graytt{wgmma\_reg\_async: } & \text{actions on wgmma register tiles by wgmma instructions}\\
\graytt{wgmma\_mem\_async: } & \text{actions on shared memory by wgmma instructions}\\
\graytt{wgmma\_reg: } & \text{actions on wgmma register tiles, by wgmma or synchronous instructions}
\end{align*}
...
\lighttt{@instr} will have to accept an optional actor kind parameter as well (defaults to CPU).

If an async actor kind is given, then child statements are considered to execute with an asynchronous actor kind, marking them as eligible for replacement with asynchronous CUDA constructs.
...
I'm proposing using the syntax ``\lighttt{x | y}'' for a synchronization statement, and that we allow declaring variables of \lighttt{barrier} type. \lighttt{x} and \lighttt{y} are the names of either actor kinds or barrier variables.

\lighttt{x : actor-kind | y : actor-kind}: non-split barrier; instructions with actor kind \lighttt{y} issued after the barrier see the actions with actor kind \lighttt{x} performed before the barrier.

\lighttt{x : actor-kind | y : barrier}: arrive

\lighttt{y : barrier | z : actor-kind}: await; instructions with actor kind \lighttt{z} issued after the await see the actions with actor kind \lighttt{x} performed before the corresponding arrive.
...
\filbreak
Implicit xedges (defined between subsequent xnodes executed with the same parlane) admit only reads and writes with a sequential actor kind, except for the special ``same matrix format'' exception for \lighttt{wgmma} accumulators.
...
\filbreak
\myKey{SyncStmt:} I'm not sure if there's a not-too-hacky way to avoid this, but for now I think we should add a new node type to LoopIR/UAST/PAST for representing synchronization statements \lighttt{x | y} (pair of symbols; either the name of a barrier variable or the name of an actor kind).
