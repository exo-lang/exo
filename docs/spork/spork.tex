...
\filbreak
\flagged{TODO} it may be time for me to admit this is probably not a good idea.
We may want to define async-blocks using Python \lighttt{with} syntax instead.
...
(Note, maybe the sync-temporal vs async-temporal distinction is not so important, but I think spatial vs temporal clearly is, and anyway the \flagged{async for} loop is going away).
...
\flagged{TODO} fat pointers e.g. tensorMap.
...
\flagged{TODO} maybe this is not a good restriction.
...
\flagged{TODO} ``lane'' name is not good.
...
\begin{enumerate}
  ...
  We need to be able to cuda-memcpy stuff (model as an \lighttt{instr} most likely) and pass kernel parameters (not sure how to best do this).
  \filbreak
  \item We will modify the LoopIR compiler to separate CPU code (top level) and CUDA code (body of a \lighttt{cuda\_clusters} or \lighttt{cuda\_blocks} parallel-for loop) and compile the latter as a separate CUDA device function.
  ...
  Both solutions have the issue that the value will appear not as normal GPU memory to Exo, despite basically being the same as an ordinary CUDA variable from the CUDA programming model's perspective, so, this could unfairly limit \lighttt{instr}s from being used.
  \filbreak
  ...
  This also greatly complicates custom \lighttt{instr}s since CUDA accelerators won't work on the CPU simulator ... could walk the AST of the \lighttt{instr}'s proc and generate CPU code from that?
  \filbreak
