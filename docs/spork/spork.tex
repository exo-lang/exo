...
\graytt{wgmma\_async\_reg: } & \text{actions on wgmma register tiles by wgmma instructions}\\
\graytt{wgmma\_async\_mem: } & \text{actions on shared memory by wgmma instructions}\\
...
I'm proposing using the syntax ``\lighttt{A // B}'' for a synchronization statement, and that we allow declaring variables of \lighttt{barrier} type. \lighttt{A} and \lighttt{B} are the names of either actor kinds or barrier variables.

\lighttt{A : actor-kind // B : actor-kind}: non-split barrier; instructions with actor kind \lighttt{B} issued after the barrier see the actions with actor kind \lighttt{A} performed before the barrier.

\lighttt{A : actor-kind // B : barrier}: arrive

\lighttt{A' : barrier // B' : actor-kind}: await; instructions with actor kind \lighttt{B'} issued after the await see the actions with actor kind \lighttt{A} performed before the corresponding arrive.
...
\filbreak
\myKey{Read/Write/Reduce:} These will not have to change in LoopIR, but the compilation of these will have to substantially modified in order to handle actor kinds and distributed memory (GPU registers).
