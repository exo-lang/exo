...
\begin{enumerate}
  ...
  \filbreak
  \item Taking a ``census'' of variables used in both CPU and CUDA code (body of a \lighttt{for cuda\_clusters} or \lighttt{for cuda\_blocks} loop) and passing them as kernel parameters.
  Not sure how to best model this either. For index/value parameters we can just pass them directly, but for numeric types it's less clear; maybe we'll allow the GPU to read (but not write) scalar values in CPU DRAM and implement this with an implicit copy ... this feels hacky though.
  \filbreak
  \item How to best modify memory types to handle the new GPU environment (can\_read must consider the current actor kind; distributed memory like GPU registers).
  \filbreak
