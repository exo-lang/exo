...
\lighttt{Sync(A1, A2)}: non-split barrier parameterized with a ``first actor kind'' \lighttt{A1} and a ``second actor kind'' \lighttt{A2}; instructions with actor kind \lighttt{A2} issued after the barrier see the actions with actor kind \lighttt{A1} performed before the barrier.
...
For split barriers, I'm expecting mostly that we will support \lighttt{cuda\_all} to \lighttt{cpu} barriers, and asynchronous CUDA to \lighttt{cuda\_sync} barriers.

\filbreak
For non-split barriers, we support the following:

\texttt{Sync(cuda\_sync, cuda\_generic)}: \lighttt{\_\_syncthreads()} and similar barriers

\texttt{Sync(wgmma\_reg, wgmma\_async\_reg)}: PTX \wgmmaFence

\texttt{Sync(cuda\_sync, wgmma\_async\_mem)}: \fenceProxyAsync\ at warpgroup scope

\texttt{Sync(cuda\_sync, tma\_to\_global\_async)}: \fenceProxyAsync\ at thread scope\footnote{Not sure about this one ... the underlying instruction is the same as the previous but \cpAsyncBulk\ instructions are issued per-thread, not per-warpgroup, so the interaction between cooperative CUDA synchronous code and TMA is quite different compared to wgmma.}

\texttt{Sync(cuda\_all, cpu)}: \lighttt{cudaStreamSynchronize()} (CPU waits for CUDA kernels)
...
for blockIdx in cuda_blocks(lo, hi, warps = 8):
    for threadIdx in cuda_threads(0, 256):
        x : f32 @ CUDA_REGISTER
        x = foo()
    Sync(cuda_sync, cuda_generic)  # __syncthreads() executed convergently by 1 block
    for threadIdx in cuda_threads(0, 256):
        y : f32 @ CUDA_REGISTER
...
\filbreak
\begin{enumerate}
  \item $V_P$ set to the write's accessor set if the instruction is asynchronous\footnote{Special behavior for wgmma needed, where sigthreads for actor signature \lighttt{sig\_wgmma\_reg\_d} go into $V_C$ despite wgmma otherwise having asynchronous semantics.}, empty otherwise;
