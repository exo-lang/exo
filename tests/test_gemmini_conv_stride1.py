...
def test_conv_3():
  ...
  #              'init_mem();',
    ...
    conv = conv.replace(ld_i8_block_id2, 'for ocol_i in _:_ #1')
    conv = conv.replace(zero_block_id2, 'for ocol_i in _:_ #1')
    ...
    conv = conv.replace(do_zero_i8_vector, 'for kch_i in _:_ #0')
    conv = conv.replace(ld_i8_block_id2, 'for ocol_i in _:_ #2')
    conv = conv.replace(zero_block_id2, 'for ocol_i in _:_ #2')
    conv = conv.reorder('kch_i', 'och_i')
    ...
    conv = conv.split('orow', 8, ['orow_o', 'orow_i'], perfect=True)
    conv = conv.par_to_seq('for krow in _:_')
    conv = conv.lift_alloc('res : _', n_lifts=1)
    conv = conv.lift_alloc('i_s : _', n_lifts=2)
    ...
    conv = conv.lift_alloc('res : _', n_lifts=3)
    conv = conv.lift_alloc('i_s : _', n_lifts=3)
    ...
    conv = conv.add_guard('for kch_o in _:_ #3', 'b', 0)
    conv = conv.add_guard('for kch_o in _:_', 'orow_o', 0)
    conv = conv.add_guard('for kch_o in _:_', 'orow_i', 0)
    conv = conv.add_guard('for kch_o in _:_ #3', 'orow_o #1', 0)
    conv = conv.add_guard('for kch_o in _:_ #3', 'orow_i #1', 0)
    ...
    #conv = conv.add_unsafe_guard('if 0 <= orow + krow - 1 and orow + krow - 1 < 56:_ #1', 'orow == 0 or krow == 2')
