...
def test_smem_linear_per_cta_negative(compiler):
    # Currently we require SMEM allocations at cluster scope due to
    # sync_check limitations. This could be changed later.
    with pytest.raises(Exception) as exc:
...
# Test modified from Claude's test.
def mkproc_missing_subdivision(subdivide_to_threads):
    ...
    subdivide_to_threads=True: iterate to thread level (valid)
    subdivide_to_threads=False: only iterate by warps (invalid)
    """
    device_fn = CudaDeviceFunction(blockDim=64)
    test_unit = cuda_thread if subdivide_to_threads else cuda_warp

    # David Zhao Akeley: Claude, I manually edited this test to fix the
    # unrelated tile_count issue, so it tests what it needs to test.
    # Note, the cuda_threads(0, 2) loop is valid -- there is no
    # requirement to use all threads.
    @proc
    def test_proc():
        with device_fn:
            for task in cuda_tasks(0, 1):
                ...
                # Valid: iterate to individual threads (box=1)
                # Invalid: only iterate by warps (box=32, not 1)
                for tid in cuda_threads(0, 2, unit=test_unit):
                    for i in seq(0, 4):
                        data[tid, i] = 1.0
...
def test_missing_subdivision_positive(compiler):
    # Valid: subdivided to thread level
    compiler.cuda_cpu_test(mkproc_missing_subdivision, subdivide_to_threads=True)


def test_missing_subdivision_negative(compiler):
    with pytest.raises(Exception) as exc:
        # Invalid: not subdivided to thread level
        compiler.cuda_cpu_test(mkproc_missing_subdivision, subdivide_to_threads=False)
    msg = str(exc.value)
