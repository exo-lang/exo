...
def test_commit_group_await_first_negative(compiler):
    ...
    msg = str(exc.value)
    assert "Distributed memory deduction for cg failed" in msg
...
def mkref_mbarriers(
    ...
    cluster_await = (
        f"mbarrier.{qc.try_or_test}_wait.parity.acquire.cluster.shared::cta.b64"
    )
    ...
    def thread_main(m_cta, n_cta, t2, t1):
        for i in range(5):
            ...
            await_impl(
                m_cta, n_cta, t2, t1, i, row_bars, 1 + f_delay, f_delay, N_CTA > 1
            )
            arrive_impl(m_cta, n_cta, t2, t1, i, match_col, col_bars, 1 + f_delay)
            await_impl(
                m_cta, n_cta, t2, t1, i, col_bars, 1 + f_delay, f_delay, M_CTA > 1
            )
            arrive_impl(m_cta, n_cta, t2, t1, i, match_any, all_bars, 1 + f_delay)
            await_impl(
                m_cta, n_cta, t2, t1, i, all_bars, 1 + f_delay, f_delay, clusterDim > 1
            )

            await_impl(
                m_cta, n_cta, t2, t1, i, f_rc_bars, b_delay + f_delay, f_delay, False
            )
...
def test_mbarriers_m4n2d1d2_in_order_to_wgmma_strict_excut(compiler_Sm90a, monkeypatch):
    ...
    compiler_Sm90a.excut_test(
        mkproc_mbarriers,
        mkref_mbarriers,
        **mb_m4n2d1d2_in_order_to_wgmma,
        strict_cluster=True,
    )


def test_mbarriers_m4n2d1d2_in_order_to_wgmma_strict_golden(
...
def test_mbarriers_strict_cluster_warning(compiler, monkeypatch):
    ...
    for value, strict in (("0", False), ("1", True), ("yes", True)):
        ...
        with warnings.catch_warnings(record=True) as record:
            ...
            src = compiler.cuda_cpu_test(
                mkproc_mbarriers, **mb_m4n2d1d2_in_order_to_wgmma
            )
        assert not [
            w for w in record if "EXO_STRICT_CLUSTER_MBARRIER" in str(w.message)
        ]
        assert (
            "mbarrier.arrive.release.cluster.shared::cluster.b64" in src.cuh_src
        ) == strict
        assert (
            "try_wait.parity.acquire.cluster.shared::cta.b64" in src.cuh_src
        ) == strict
    ...
    def proc_no_trailing_barrier(gmem: f32[128] @ CudaGmemLinear):
        with CudaDeviceFunction(blockDim=32):
            for task in cuda_tasks(0, 1):
                ...
                bar: barrier[1 @ ring_buffer_by(1)] @ Sm80_CommitGroup
                ...
                Arrive(Sm80_cp_async, 1) >> bar[0]
                Await(bar[0], cuda_in_order, 0)
    ...
    with pytest.raises(Exception) as exc:
        ...
        def test_proc(xyzzy: f32):
            with CudaDeviceFunction(clusterDim=8, blockDim=256):
                for task in cuda_tasks(0, 4):
                    bars: barrier[2, 4 @ ring_buffer_by(4)] @ CudaMbarrier
...
def mkproc_broken_barrier_exprs(bug):
    ...
    def proc_broken_barrier_exprs():
        with CudaDeviceFunction(clusterDim=4, blockDim=128):
            for task in cuda_tasks(0, 4):
                mbar_a: barrier[2, 2, 1 @ ring_buffer_by(1)] @ CudaMbarrier
                mbar_b: barrier[2, 2, 1 @ ring_buffer_by(1)] @ CudaMbarrier
                for m_cta in cuda_threads(0, 2, unit=2 * cuda_cta_in_cluster):
                    for n_cta in cuda_threads(0, 2, unit=cuda_cta_in_cluster):
                        if bug == home_barrier_missing_vars_bug:
                            Arrive(cuda_in_order) >> mbar_a[m_cta, :, 0]
                        else:
                            (
                                Arrive(cuda_in_order)
                                >> mbar_a[m_cta, :, 0]
                                >> mbar_a[m_cta, n_cta, 0]
                            )
                        if bug == multicast_await_bug:
                            Await(mbar_a[m_cta, :, 0], cuda_in_order, 0)
                        else:
                            Await(mbar_a[m_cta, n_cta, 0], cuda_in_order, 0)
                        ...
                        if bug == home_barrier_binop_bug:
                            Await(mbar_b[m_cta, n_cta / 2, 0], cuda_in_order, 0)
                        else:
                            Await(mbar_b[m_cta, n_cta, 0], cuda_in_order, 0)
...
def test_home_barrier_binop(compiler):
    ...
    msg = str(exc.value)
    assert "single variable name" in msg
...
def test_await_wrong_N(compiler):
    ...
    helper(CudaClusterSync, +1, +1, "N = 0")
    helper(CudaMbarrier, 0, 0, "@ ring_buffer_by")
    helper(CudaMbarrier, -1, -1, "N = 0")
    helper(CudaMbarrier, +1, +1, "N = 0")
