...
// This manipulates thread_local state; the same CUDA device must be used
// throughout. We use the cudaStreamLegacy for async malloc/free
...
void exo_excut_end_log_action(const char *device_name, unsigned _blockIdx,
    unsigned _threadIdx, const char *file, int line);
exo_ExcutDeviceLog exo_excut_get_device_log();
void exo_excut_flush_device_log(cudaStream_t exo_cudaStream, unsigned gridDim,
    unsigned blockDim, uint32_t str_id_count, const char *const *str_table,
    uint32_t file_id_count, const char *const *file_table);
#define EXO_EXCUT_DEVICE_LOG_MEMBER exo_ExcutDeviceLog exo_excutDeviceLog;
...
struct exo_ExcutThreadLog {
  ...
  EXO_EXCUT_CUDA_INLINE void log_action(
      uint32_t action_str_id, uint32_t file_id, uint32_t line) const {
    if (uint32_t *p = _alloc_log(3)) {
      p[0] =
          uint32_t(exo_ExcutHighByte::action) << 24 | action_str_id & 0xFFFFFF;
      p[1] = file_id;
      p[2] = line;
    }
  }
  ...
  template <typename T>
  EXO_EXCUT_CUDA_INLINE void log_ptr_data_arg(
      const T *data, uint32_t count = 1) {
    static_assert(sizeof(T) % 4u == 0);
    const uint32_t n_value_words = sizeof(T) * count / 4u;
    if (uint32_t *p = _alloc_log(n_value_words + 1)) {
      p[0] = uint32_t(exo_ExcutHighByte::int_arg) << 24 |
             (n_value_words * 4u) & 0xFFFFFF;
      uint32_t *payload = p + 1;
      for (uint32_t i = 0; i < n_value_words; ++i) {
        payload[i] = reinterpret_cast<const uint32_t *>(
            data)[i];  // Strict aliasing violation
      }
    }
  }
};

EXO_EXCUT_CUDA_INLINE exo_ExcutThreadLog exo_excut_begin_thread_log(
    exo_ExcutDeviceLog device_log) {
  uint32_t n_threads = gridDim.x * blockDim.x;
  uint32_t tid = blockDim.x * blockIdx.x + threadIdx.x;
  uint32_t words_per_thread = device_log.word_capacity / n_threads;
  exo_ExcutThreadLog thread_log{
      device_log.data + words_per_thread * tid, words_per_thread};
  if (words_per_thread != 0) {
