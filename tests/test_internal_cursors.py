...
def test_insert_root_front(proc_foo, golden):
    ...
    foo2, _ = c.body().before().insert([LoopIR.Pass(None, c._node().srcinfo)])
    assert str(foo2) == golden


def test_insert_root_end(proc_foo, golden):
    c = Cursor.root(proc_foo)
    foo2, _ = c.body().after().insert([LoopIR.Pass(None, c._node().srcinfo)])
    assert str(foo2) == golden
...
def test_block_replace(proc_bar, golden):
    ...
    bar2, _ = stmts.replace([LoopIR.Pass(None, c._node().srcinfo)])
    assert str(bar2) == golden
...
def test_node_replace(proc_bar, golden):
    ...
    bar2, _ = c.replace([LoopIR.Pass(None, c._node().srcinfo)])
    assert str(bar2) == golden
...
def test_cursor_replace_expr(proc_foo, golden):
    c = proc_foo._TEST_find_cursors("m")[0]
    foo2, _ = c.replace(LoopIR.Const(42, T.size, c._node().srcinfo))
    assert str(foo2) == golden
...
def test_block_replace_forward_gap(proc_bar, old, new):
    ...
    if new is None:
        ...
        new_pat, new_gap = new
        new_c = bar_new._TEST_find_stmt(new_pat)
...
def test_block_move_identity_1(proc_bar):
    for_j = proc_bar._TEST_find_stmt("for j in _: _").body()
    bar_new, fwd = for_j[0:3].move_to(for_j[0].before())
...
def test_block_move_identity_2(proc_bar):
    for_j = proc_bar._TEST_find_stmt("for j in _: _").body()
    bar_new, fwd = for_j[0:3].move_to(for_j[3].before())
...
def test_block_move_within_block(proc_bar, golden):
    for_j = proc_bar._TEST_find_stmt("for j in _: _").body()
    bar_new, fwd = for_j[0:3].move_to(for_j[-1].after())
    print(bar_new)
...
def test_block_move_before_parent(proc_bar, golden):
    for_j = proc_bar._TEST_find_stmt("for j in _: _").body()
    bar_new, fwd = for_j[0:3].move_to(for_j.parent().before())
    print(bar_new)
    assert str(bar_new) == golden
...
def test_block_move_after_parent(proc_bar, golden):
    for_j = proc_bar._TEST_find_stmt("for j in _: _").body()
    bar_new, fwd = for_j[0:3].move_to(for_j.parent().after())
    print(bar_new)
    assert str(bar_new) == golden
