...
def test_blur():
    ...
    k_size = 3
    kernel = nparray([[1/16.0,1/8.0,1/16.0],[1/8.0,1/4.0,1/8.0],[1/16.0,1/8.0,1/16.0]])
    res = nprand(size=(n_size,m_size))
    res_c = cvt_c(res)
    test_lib.blur(c_int(n_size), c_int(m_size), c_int(k_size), cvt_c(image), cvt_c(kernel), res_c)
