...
def test_double_fission(golden):
    ...
    foo = foo.lift_alloc('res : _', keep_dims=True)
...
def test_simple_lift_alloc(golden):
    ...
    bar = bar.lift_alloc('tmp_a : _', n_lifts=1, keep_dims=True)
    assert str(bar) == golden
...
def test_lift(golden):
    ...
    bar = bar.lift_alloc('a: i8[_]', n_lifts=1, mode='col', size=20, keep_dims=True)
    assert str(bar) == golden
