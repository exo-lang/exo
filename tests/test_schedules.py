...
def test_repeat():
    ...
    c = bar.find("tmp_a: _")
    print(bar)
    print(repeated_lift_alloc(bar, c))
...
def nlr(c):
    for c in c.body():
        yield c
        if isinstance(c, (ForSeqCursor, IfCursor)):
            yield from nlr(c)


def lrn(c):
    for c in reversed(list(c.body())):
        if isinstance(c, (ForSeqCursor, IfCursor)):
            yield from lrn(c)
        yield c
...
def test_reduce():
    ...
    print(reduce(lift_alloc, traversal_func)(bar, y_c))
...
def test_nlr():
    ...
    [print(p) for p in lrn(bar)]


def try_do(sched):
    def func(proc, *args, **kwargs):
        try:
            proc = sched(proc, *args, **kwargs)
        except:
            pass
        return proc
...
def test_reduce2():
    ...
    print(reduce(lift_alloc, traversal_func)(bar, y_c)[0])
...
def test_try():
    ...
    print(repeat(lift_alloc)(bar, a_c)[0])
    ...
    # print(repeat(try_else(seq(fission_after, remove_parent_loop), reorder_before))(bar, x_c)[0])

    # reorder_before     = save_cursor(seq(reframe(lambda c:c.expand(1, 0)), reorder_stmts))

    # print(repeat(reorder_before)(bar, x_c)[0])
    # print(seq(fission_after, remove_parent_loop)(bar, x_c)[0])
    # print(try_else(seq(fission_after, remove_parent_loop), reorder_before)(bar, x_c)[0])

    # print(repeat(try_else(seq(fission_after, remove_parent_loop), reorder_before2))(bar, x_c)[0])

    # print(reorder_before(bar, x_c)[0])

    # print(repeat(seq(seq(repeat(reorder_before2), fission_after), remove_parent_loop))(bar, x_c)[0])
