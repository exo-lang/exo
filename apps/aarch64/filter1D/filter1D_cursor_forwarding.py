...
y_c = filter1D.forward(sum_c).next().next().body()[-1]
filter1D = fission(filter1D, y_c.before())

# stage x
filter1D = reorder_loops(filter1D, "outXi k")
filter1D = bind_expr(filter1D, "x[_]", "xX4")
filter1D = expand_dim(filter1D, "xX4:_", "4", "outXi")
filter1D = lift_alloc(filter1D, "xX4:_")
filter1D = fission(filter1D, filter1D.find("xX4[_] = _").after())
