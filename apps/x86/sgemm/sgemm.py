...
def schedule_kernel(p, cond, M, N):
    ...
    for it in ("i1", "j", "i1"):
        p = vectorize(
            p, p.find_loop(it), VEC_W, "f32", AVX512, rules=rules, tail="perfect"
        )
...
def make_sgemm_exo(p=SGEMM):
    ...
    for i in range(0, 8):
        B_name = "B_cache"
        p, (alloc, _, _, _) = auto_stage_mem(
            p, p.find_loop(f"ki #{i}"), "B", B_name, rc=True
        )
        p = resize_dim(p, alloc, 0, K_L1_BLK, 0)
        p = resize_dim(p, alloc, 1, N_L1_BLK, 0)
        p = set_memory(p, alloc, DRAM_STATIC)
    p, (alloc, _, _, _) = auto_stage_mem(p, p.find_loop("jo"), "A", "A_cache", rc=True)
