...
void conv_oneDNN(benchmark::State &state) {
  ...
  auto user_src_mem = memory({ci.src_dims, dt::f32, tag::nchw}, engine);
  auto user_weights_mem = memory({ci.weights_dims, dt::f32, tag::oihw}, engine);
  auto user_dst_mem = memory({ci.dst_dims, dt::f32, tag::nchw}, engine);
