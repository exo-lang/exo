...
//     inp : ui8[n, m] @DRAM
// )
void blur_inlined( void *ctxt, int_fast32_t n, int_fast32_t m, uint8_t* g, const uint8_t* inp ) {
EXO_ASSUME(n > 5);
EXO_ASSUME(m > 5);
for (int_fast32_t i = 0; i < -4 + n; i++) {
  for (int_fast32_t j = 0; j < -4 + m; j++) {
    g[i * m + j] = ((inp[i * m + j] + inp[i * m + 1 + j] + inp[i * m + 2 + j] + inp[i * m + 3 + j] + inp[i * m + 4 + j]) / 5.0 + (inp[(1 + i) * m + j] + inp[(1 + i) * m + 1 + j] + inp[(1 + i) * m + 2 + j] + inp[(1 + i) * m + 3 + j] + inp[(1 + i) * m + 4 + j]) / 5.0 + (inp[(2 + i) * m + j] + inp[(2 + i) * m + 1 + j] + inp[(2 + i) * m + 2 + j] + inp[(2 + i) * m + 3 + j] + inp[(2 + i) * m + 4 + j]) / 5.0 + (inp[(3 + i) * m + j] + inp[(3 + i) * m + 1 + j] + inp[(3 + i) * m + 2 + j] + inp[(3 + i) * m + 3 + j] + inp[(3 + i) * m + 4 + j]) / 5.0 + (inp[(4 + i) * m + j] + inp[(4 + i) * m + 1 + j] + inp[(4 + i) * m + 2 + j] + inp[(4 + i) * m + 3 + j] + inp[(4 + i) * m + 4 + j]) / 5.0) / 5.0;
  }
}
}
