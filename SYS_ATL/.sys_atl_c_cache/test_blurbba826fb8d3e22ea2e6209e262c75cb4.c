...
for (int xhi=0; xhi < _ceil_div(N - 8 + 1, 8); xhi++) {
      ...
      for (int j=0; j < K; j++) {
        for (int xlo=0; xlo < 8; xlo++) {
          for (int ylo=0; ylo < 16; ylo++) {
            if (8 * xhi + xlo + i < N && 16 * yhi + ylo + j < M) {
              res[(8 * xhi + xlo) * M + (16 * yhi + ylo)] += kernel[(i) * K + (j)] * image[(8 * xhi + xlo + i) * M + (16 * yhi + ylo + j)];
            }
          }
        }
      }
  ...
  for (int xlo=0; xlo < 8; xlo++) {
    for (int ylo=0; ylo < M - _ceil_div(M - 16 + 1, 16) * 16; ylo++) {
      for (int i=0; i < K; i++) {
        for (int j=0; j < K; j++) {
          if (8 * xhi + xlo + i < N && ylo + _ceil_div(M - 16 + 1, 16) * 16 + j < M) {
            res[(8 * xhi + xlo) * M + (ylo + _ceil_div(M - 16 + 1, 16) * 16)] += kernel[(i) * K + (j)] * image[(8 * xhi + xlo + i) * M + (ylo + _ceil_div(M - 16 + 1, 16) * 16 + j)];
          }
        }
      }
    }
