...
for (int xhi=0; xhi < _ceil_div(N - 32 + 1, 32); xhi++) {
      ...
      for (int j=0; j < K; j++) {
        for (int xlo=0; xlo < 32; xlo++) {
          for (int ylo=0; ylo < 64; ylo++) {
            if (32 * xhi + xlo + i < N && 64 * yhi + ylo + j < M) {
              res[(32 * xhi + xlo) * M + (64 * yhi + ylo)] += kernel[(i) * K + (j)] * image[(32 * xhi + xlo + i) * M + (64 * yhi + ylo + j)];
            }
          }
        }
      }
  ...
  for (int xlo=0; xlo < 32; xlo++) {
    for (int ylo=0; ylo < M - _ceil_div(M - 64 + 1, 64) * 64; ylo++) {
      for (int i=0; i < K; i++) {
        for (int j=0; j < K; j++) {
          if (32 * xhi + xlo + i < N && ylo + _ceil_div(M - 64 + 1, 64) * 64 + j < M) {
            res[(32 * xhi + xlo) * M + (ylo + _ceil_div(M - 64 + 1, 64) * 64)] += kernel[(i) * K + (j)] * image[(32 * xhi + xlo + i) * M + (ylo + _ceil_div(M - 64 + 1, 64) * 64 + j)];
          }
        }
      }
    }
