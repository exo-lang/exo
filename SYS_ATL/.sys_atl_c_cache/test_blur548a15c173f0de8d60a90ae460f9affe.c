...
for (int xhi=0; xhi < _ceil_div(N - 16 + 1, 16); xhi++) {
      ...
      for (int j=0; j < K; j++) {
        for (int xlo=0; xlo < 16; xlo++) {
          for (int ylo=0; ylo < 32; ylo++) {
            if (16 * xhi + xlo + i < N && 32 * yhi + ylo + j < M) {
              res[(16 * xhi + xlo) * M + (32 * yhi + ylo)] += kernel[(i) * K + (j)] * image[(16 * xhi + xlo + i) * M + (32 * yhi + ylo + j)];
            }
          }
        }
      }
  ...
  for (int xlo=0; xlo < 16; xlo++) {
    for (int ylo=0; ylo < M - _ceil_div(M - 32 + 1, 32) * 32; ylo++) {
      for (int i=0; i < K; i++) {
        for (int j=0; j < K; j++) {
          if (16 * xhi + xlo + i < N && ylo + _ceil_div(M - 32 + 1, 32) * 32 + j < M) {
            res[(16 * xhi + xlo) * M + (ylo + _ceil_div(M - 32 + 1, 32) * 32)] += kernel[(i) * K + (j)] * image[(16 * xhi + xlo + i) * M + (ylo + _ceil_div(M - 32 + 1, 32) * 32 + j)];
          }
        }
      }
    }
