...
for (int xhi=0; xhi < _ceil_div(N - 4 + 1, 4); xhi++) {
      ...
      for (int j=0; j < K; j++) {
        for (int xlo=0; xlo < 4; xlo++) {
          for (int ylo=0; ylo < 8; ylo++) {
            if (4 * xhi + xlo + i < N && 8 * yhi + ylo + j < M) {
              res[(4 * xhi + xlo) * M + (8 * yhi + ylo)] += kernel[(i) * K + (j)] * image[(4 * xhi + xlo + i) * M + (8 * yhi + ylo + j)];
            }
          }
        }
      }
  ...
  for (int xlo=0; xlo < 4; xlo++) {
    for (int ylo=0; ylo < M - _ceil_div(M - 8 + 1, 8) * 8; ylo++) {
      for (int i=0; i < K; i++) {
        for (int j=0; j < K; j++) {
          if (4 * xhi + xlo + i < N && ylo + _ceil_div(M - 8 + 1, 8) * 8 + j < M) {
            res[(4 * xhi + xlo) * M + (ylo + _ceil_div(M - 8 + 1, 8) * 8)] += kernel[(i) * K + (j)] * image[(4 * xhi + xlo + i) * M + (ylo + _ceil_div(M - 8 + 1, 8) * 8 + j)];
          }
        }
      }
    }
