"""
This module defines special symbols that are used inside SYS_ATL code. You may
import this module via `from SYS_ATL.syntax import *` to suppress warnings and
...
def _fail_dummy():
    raise SyntaxError('SYS_ATL Python dummy functions should never be called '
                      'directly.')
...
class size(int):
    """
    A type representing a size in SYS_ATL.
    """

    def __init__(self, *_):
        raise SyntaxError('SYS_ATL Python dummy objects should never be '
                          'instantiated')
