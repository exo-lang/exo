...
class Parser:
  def __init__( self, module_ast, func_globals, srclocals, getsrcinfo,
    ...
    #elif as_quote:
    #  self._cached_result = self.parse_fdef(module_ast)
    else: assert False, "parser mode configuration unsupported"
    self.locals.pop()
  ...
  def parse_fdef(self, fdef):
    ...
    for a in fargs.args:
      ...
      if type(tnode) is pyast.Name and tnode.id == 'size':
        if len(args) > 0:
          self.err(a, f"sizes must be declared before all other arguments")
        if a.arg in names:
  ...
  def parse_arg_type(self, node):
    ...
    elif node.right.id == "IN":       eff = T.In
    elif node.right.id == "OUT":      eff = T.Out
    elif node.right.id == "INOUT":    eff = T.InOut
    else:
      self.err(node.right, eff_err_str)
  ...
  def parse_type(self, node):
    if type(node) is pyast.Subscript:
      if type(node.value) is not pyast.Name or node.value.id != "R":
        self.err(node, "expected tensor type to be of the form 'R[...]'")

      # unpack single or multi-arg indexing to list of slices/indices
      if type(node.slice) is pyast.ExtSlice:
          ...
          if type(idx.value) is pyast.Constant:
            ...
            if idx.value.id in self.locals:
              ...
              if type(sz) is SizeStub:
                ...
                continue
        # error fall-through
        self.err(idx, "expected positive integer constant or size variable")
      ...
      if (type(s) is pyast.Assign or type(s) is pyast.AnnAssign or
        ...
        if type(s) is pyast.AnnAssign:
          if s.value is not None:
            self.err(s, "Variable declaration should not have value assigned")
        else:
  ...
  def parse_array_indexing(self, node):
    if type(node) is pyast.Name:
      ...
      def parse_idx(e):
        if type(e) is not pyast.Index:
          self.err(e, "slicing notation such as x[ a : b ] not allowed")
  ...
  def parse_expr(self, e):
    if type(e) is pyast.Name or type(e) is pyast.Subscript:
      ...
      if   type(e.op) is pyast.Add:       op = "+"
      elif type(e.op) is pyast.Sub:       op = "-"
      elif type(e.op) is pyast.Mult:      op = "*"
      elif type(e.op) is pyast.Div:       op = "/"
      elif type(e.op) is pyast.FloorDiv:  op = "//"
      elif type(e.op) is pyast.Mod:       op = "%"
      elif type(e.op) is pyast.Pow:       op = "**"
      elif type(e.op) is pyast.LShift:    op = "<<"
      elif type(e.op) is pyast.RShift:    op = ">>"
      elif type(e.op) is pyast.BitOr:     op = "|"
      elif type(e.op) is pyast.BitXor:    op = "^"
      elif type(e.op) is pyast.BitAnd:    op = "&"
      elif type(e.op) is pyast.MatMult:   op = "@"
      else: assert False, "unrecognized op"
      ...
      if   type(e.op) is pyast.And:   op = "and"
      elif type(e.op) is pyast.Or:    op = "or"
      else: assert False, "unrecognized op"
      ...
      for opnode,lhs,rhs in zip(e.ops, vals[:-1], vals[1:]):
        if   type(opnode) is pyast.Eq:    op = "=="
        elif type(opnode) is pyast.NotEq: op = "!="
        elif type(opnode) is pyast.Lt:    op = "<"
        elif type(opnode) is pyast.LtE:   op = "<="
        elif type(opnode) is pyast.Gt:    op = ">"
        elif type(opnode) is pyast.GtE:   op = ">="
        elif type(opnode) is pyast.Is:    op = "is"
        elif type(opnode) is pyast.IsNot: op = "is not"
        elif type(opnode) is pyast.In:    op = "in"
        elif type(opnode) is pyast.NotIn: op = "not in"
        else: assert False, "unrecognized op"
        if op not in front_ops:
          self.err(e, f"unsupported binary operator: {op}")
